...
**基线：** `5a24d610a612d849767e9cfecb1a3a89ea135428`
...
- 报价 API 自身存在审计/任务副作用，因此不能描述为“纯只读”；`notify_email=false`、`notify_wecom=false` 只关闭通知，不消除上游副作用。
...
| --- | --- | --- | --- |
| AI 报价 | `POST /quotes/zone-calculate`；请求必须有 `cbm`；`notify_email` 和 `notify_wecom` 必须为 `false`；响应字段已核验，缺口见下 | `cargo.total_volume` 归一为 API 的 `cbm`；缺失时 `needs_input`，不猜方数；保留真实响应的报价字段 | 实际 base URL、认证 header 和副作用明细仍须用隔离合同核验；已核验响应缺少现行 QuoteResult 所需的四个证据字段 |
...
报价 response 已核验字段为：`quote_id`、`source_type`、`confidence`、`postal`、`city`、`province`、`origin`、`zone`、`billing_pallets`、`base_price_usd`、`fuel_usd`、`accessorials`、`total_price_usd`、`risk_tags`、`manual_review_required`、`matched_rule`、`matched_by`、`candidate_count`、`match_trace`。该响应没有 `rule_version`、`data_version`、`valid_from`、`valid_to`。
...
已核验报价响应缺少 `rule_version`、`data_version`、`valid_from`、`valid_to` 四个证据，所以在现行 `quoteResultSchema` 下不能映射为 `success`。11B 前需要另行 RFC 决定如何表达 adapter contract version 和 canonical response hash；不得把它们假填为业务规则版本、数据版本或有效期。

## 4. 版本、来源和哈希

- 每次请求直连当前 API，不缓存旧响应，不轮询、不排队；重试只服从安全 HTTP 客户端和上游明确合同。
- 报价目标是记录 MCP adapter contract version 与 canonical response 的 SHA-256；在承载方式经另行 RFC 接受前，不改写现行 QuoteResult 字段，也不得把它们写成上游业务 `rule_version`、`data_version` 或有效期。
...
| 合同、来源、权限和响应字段均通过校验 | `success` | 报价仍 `sendable=false`；关务仍是候选，不是正式归类 |
...
3. 只接受真实响应中的字段；待 adapter contract/hash 的 RFC 接受后记录 adapter contract version + canonical response SHA-256；不写报价草稿、不发送通知。
