...
- 影响范围：模块控制元数据、Admin 管理 API、静态模块启停门禁、构建清单、发布与回滚 runbook
...
6. 管理写入要求 admin role、`platform:admin` scope 和服务端配置的管理 tenant。preview creator
...
`docs/contracts/**` 不在本 RFC 实施中修改。如果后续要新增公共 MCP 管理工具或改变业务工具
...
- Admin API 外层 `status` 复用统一五状态词汇；内部 registration/release/readback 状态只在
  `data` 中表达。
- 没有 active release 时默认启用当前全部静态模块，保持升级前行为。
...
- `/admin` 首版继续仅回环访问；生产多人访问仍需批准的企业身份网关，未在本 RFC 中授权
  直接公网暴露。
- 所有写 API 检查 loopback、Host、Origin、Content-Type、body size、Bearer、tenant、role、
  scope、schema 和幂等键，检查顺序发生在业务写入之前。
- token 只在浏览器内存和请求 Authorization header 中存在，不持久化、不回显、不审计。
- 生产不接受 fixture token；fixture 两身份只在回环 fixture assembly 显式配置。
...
- approval：另一 actor 决定；reject/expired/consumed 不可发布。
...
3. 旧代码不认识 control DB 时忽略该独立文件，现有静态模块默认全部启用。
