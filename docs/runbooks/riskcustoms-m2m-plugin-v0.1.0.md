...
| 认证注入 | 服务端从 `MCP_RISK_CUSTOMS_AUTH_SECRET_FILE` 引用的普通 secret 文件做拒绝符号链接、非普通文件和大于 8 KiB 的有界读取；不接受客户端 token，不写入日志或仓库 |
| tenant 注入 | 从已验证的 `ExecutionContext.tenantId` 发送 `X-Tenant-Id`；必须精确命中 `MCP_RISK_CUSTOMS_ALLOWED_TENANTS`，不接受 wildcard，客户端不能覆盖；该本地白名单不替代 RiskCustoms 上游 token-to-tenant 授权映射 |
...
MCP_RISK_CUSTOMS_ALLOWED_TENANTS=<approved-tenant-id>[,<approved-tenant-id>]
...
Compose 文件、客户端配置或 PR。启用时本地 tenant 白名单不能为空且不接受 `*`；它只能缩小
MCP 可转发的 tenant 集合，不能作为上游已经发布 token-to-tenant 授权的证据。
