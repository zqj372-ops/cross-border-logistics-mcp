...
版本：0.002。入口为 `deploy/cli`，二进制命令为 `freightclaw`。这是现有固定 REST 路由的客户端，适用已接受的 [统一应用 Key 合同](../rfcs/2026-09-06-unified-application-key-v1.md)；没有新增服务端模块、工具合同、身份或业务写权限。
...
npm run build:cli
npm pack ./dist/cli
npm install --global /absolute/path/freightclaw-cli-0.002.tgz
freightclaw --version
