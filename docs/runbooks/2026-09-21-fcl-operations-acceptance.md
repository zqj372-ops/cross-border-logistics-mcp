...
批量请求可携带 `estimate_request`，首次调价即可创建多个目的地方案；不带时只重算已有方案。新估价请求的柜数要求显式 `unit: "CNTR"`，原询价/Case 接口保持兼容。保持 receiver/session/CSRF/幂等约束；API Key 不获得个人工作区权限。独立 CLI 包现有 120 个 workspace 命令，其中 36 个 FCL staff 动作。
...
幂等记录绑定原始估价版本，后续调价和重启不改变重放金额；批量发布重放返回原发布版本。旧附加费使用稳定身份映射，删除/重排不会把人工售价套到其他费用。120 行计算上限明确阻断并清空总额，不返回截断的有效报价。对应回归覆盖单位缺失、历史重试、费用重排/重复、超大配置及客户报价行映射。
