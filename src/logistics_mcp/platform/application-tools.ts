...
export const SCHEDULE_MCP_PROFILE = "schedule-live-v1" as const;
...
export const SCHEDULE_MCP_TOOLS = Object.freeze([
  "maritime.schedule.carriers",
  "maritime.schedule.locations",
  "maritime.schedule.search",
] as const);
export const ALL_MCP_TOOLS = Object.freeze([
  ...BUSINESS_MCP_TOOLS,
  ...SCHEDULE_MCP_TOOLS,
] as const);
...
export type ScheduleMcpTool = typeof SCHEDULE_MCP_TOOLS[number];
export type McpTool = typeof ALL_MCP_TOOLS[number];
...
export function isScheduleMcpIdentity(input: { role: unknown; roles: unknown; scopes: unknown; profile?: unknown }): boolean {
  return input.profile === SCHEDULE_MCP_PROFILE && input.role === "service"
    && Array.isArray(input.roles) && input.roles.length === 1 && input.roles[0] === "service"
    && Array.isArray(input.scopes) && input.scopes.length > 0
    && input.scopes.length <= SCHEDULE_MCP_TOOLS.length
    && new Set(input.scopes).size === input.scopes.length
    && input.scopes.every(scope => typeof scope === "string" && SCHEDULE_MCP_TOOLS.some(tool => scope === `tool:${tool}`));
}
