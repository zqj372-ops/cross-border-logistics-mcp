...
const {chromium}=await import(pathToFileURL(process.env.PLAYWRIGHT_MODULE).href),browser=await chromium.launch({headless:true,chromiumSandbox:true,executablePath:process.env.PLAYWRIGHT_EXECUTABLE_PATH});
...
try{
 ...
 let rendererUnavailable=null,linkedCacheVerified=false,unlinkedPdfVerified=false;
 ...
 const disableFailure=key=>{const entry=expectedFailures.find(value=>value.key===key);assert.ok(entry,'unknown expected failure '+key);entry.disabled=true;};
 ...
 // newly approved linked document: real renderer success must be reusable through the CLI cache path
 expectedFailures.find(entry=>entry.key==='newly-approved-formal').id=secondSaved.id;
 const linkedDownloadEvent=page.waitForEvent('download').catch(()=>null),rendererWait=exportWaiter(secondSaved.id);await page.getByRole('button',{name:'导出 PDF',exact:true}).click();
 const rendererReply=await rendererWait;
 if(rendererReply.status()===503){const rendererBody=await recordFailure(rendererReply,'newly-approved-formal');assert.equal(rendererBody.status,'unavailable');assert.equal(rendererBody.data,null);rendererUnavailable='linked formal export returned 503 '+rendererBody.reason_codes.join(',');}
 else{
  disableFailure('newly-approved-formal');assert.equal(rendererReply.status(),200);const rendererBody=await rendererReply.json();assert.equal(rendererBody.status,'success');assert.equal(rendererBody.data.draft,false);assert.equal(rendererBody.data.historical,false);assert.equal(rendererBody.data.valid_now,true);
  const download=await linkedDownloadEvent;assert.ok(download,JSON.stringify(rendererBody));const webPath=resolve(out,'linked-approved-web.pdf');await download.saveAs(webPath);const webBytes=readFileSync(webPath),responseBytes=Buffer.from(rendererBody.data.content_base64,'base64');
  assert.equal(webBytes.length,rendererBody.data.byte_length);assert.equal(sha256(webBytes),rendererBody.data.sha256);assert.deepEqual(webBytes,responseBytes);
  const cachedPath=resolve(out,'linked-approved-cli-cache.pdf'),cached=cli(['documents','export','--file',cachedPath],{contract_version:LINK,id:secondSaved.id,mode:'formal'});assert.equal(cached.code,0,JSON.stringify(cached));const cachedBytes=readFileSync(cachedPath);
  assert.equal(cached.data.data.sha256,rendererBody.data.sha256);assert.equal(cached.data.data.byte_length,webBytes.length);assert.equal(sha256(cachedBytes),rendererBody.data.sha256);assert.deepEqual(cachedBytes,webBytes);linkedCacheVerified=true;
 }
 ...
 if(draftReply.status()===503){await recordFailure(draftReply,'unlinked-draft-formal');rendererUnavailable=[rendererUnavailable,'draft export returned '+draftReply.status()+' '+draftBody.reason_codes.join(',')].filter(Boolean).join('; ');}
 else{disableFailure('unlinked-draft-formal');assert.equal(draftReply.status(),200);assert.equal(draftBody.status,'success');assert.equal(draftBody.data.draft,true);const download=await downloadEvent;assert.ok(download,JSON.stringify(draftBody));const draftPath=resolve(out,'draft-quotation.pdf');await download.saveAs(draftPath);const draftBytes=readFileSync(draftPath),responseBytes=Buffer.from(draftBody.data.content_base64,'base64');assert.equal(draftBytes.length,draftBody.data.byte_length);assert.equal(sha256(draftBytes),draftBody.data.sha256);assert.deepEqual(draftBytes,responseBytes);unlinkedPdfVerified=true;}
 ...
 if(!unlinkedPdfVerified){const failed=cli(['documents','export','--file',resolve(out,'approved-quotation.pdf')],{id:saved.id});assert.notEqual(failed.code,0,'renderer unavailable must not report success');assert.equal(existsSync(resolve(out,'approved-quotation.pdf')),false,'failed export must not create a file');rendererUnavailable=[rendererUnavailable,'CLI formal export fail-closed verified'].filter(Boolean).join('; ');}
 else{const approvedPath=resolve(out,'approved-quotation.pdf'),exported=cli(['documents','export','--file',approvedPath],{id:saved.id});assert.equal(exported.code,0,JSON.stringify(exported));assert.equal(exported.data.data.draft,false);const approvedBytes=readFileSync(approvedPath),approvedHash=sha256(approvedBytes);assert.equal(exported.data.data.sha256,approvedHash);assert.equal(exported.data.data.byte_length,approvedBytes.length);assert.equal(statSync(approvedPath).mode&0o777,0o600);assert.equal(cli(['documents','export','--file',approvedPath],{id:saved.id}).code,1,'must not overwrite existing file');assert.deepEqual(readFileSync(approvedPath),approvedBytes);assert.equal(sha256(readFileSync(approvedPath)),approvedHash);}
 ...
 for(const observed of resolvedFailures.filter(observed=>observed.status>=500&&!quotaResolved.includes(observed))){const match=expectedFailures.find(entry=>!entry.disabled&&entry.status>=500&&entry.method===observed.method&&entry.path===observed.path&&entry.status===observed.status&&entry.schema===observed.schema&&entry.reasons.length===observed.reasons.length&&entry.reasons.every((reason,index)=>reason===observed.reasons[index])&&entry.id===observed.id&&entry.mode===observed.mode&&entry.version===observed.version);assert.ok(match,'unexpected 5xx '+JSON.stringify(observed));match.observed=(match.observed||0)+1;}
 for(const entry of expectedFailures.filter(value=>!value.disabled)){if(entry.status>=500)assert.equal(entry.observed,entry.count,`${entry.key} expected ${entry.count} x ${entry.status} ${entry.path}`);else assert.equal(entry.seen,entry.count,`${entry.key} expected ${entry.count} business outcome`);}
 ...
 const passed=['login guard','blank template','web template save and readback','web decimal preview 250.30 USD','draft save and CLI readback','published synthetic residential release and quote probe','linked case review context, confirmation gate and hand-off','linked prepare/save with CLI same-id readback','linked approve','supplement change blocks the copied linked draft until re-confirmation','manual approval with evidence','market configuration entry','desktop/mobile no horizontal overflow','linked old-supplement formal = 200 manual_review case_review_required','history missing = 503 inquiry_quote_history_bytes_missing','approve + list failure keeps the confirmed result','linked reject + CLI readback','non-manager cannot see owner case or linked record','late committed response withheld across an identity switch','initial list failure is not an empty history','revoked member approve/get/list deny with exact v2 blocked envelopes','second organization session switch with old case/quote/confirmation isolation','switch back does not restore the previous confirmation or draft marker',...(linkedCacheVerified?['linked formal Web export + CLI cached readback with identical SHA-256']:['newly approved formal = 503 document_renderer_unavailable']),...(unlinkedPdfVerified?['web draft PDF download with response length/SHA-256 readback','CLI formal PDF download with SHA-256 and mode 0600','refuse existing output overwrite without content change']:['renderer unavailable → draft/formal export fail-closed without files'])];
 const notVerified=[...(!linkedCacheVerified?['linked formal Web export + CLI cached readback with identical SHA-256']:[]),...(!unlinkedPdfVerified?['web draft PDF download','CLI formal PDF download and SHA-256/mode 0600','refuse existing output overwrite without content change']:[])];
 console.log(JSON.stringify({rendererUnavailable,linkedCacheVerified,unlinkedPdfVerified,unsavedInputDialogObserved,observedFailures:resolvedFailures,expectedFailures,rendererNetworkErrors,passed,notVerified,errors,expectedUnavailable,artifacts:out}));
