...
it('selects by shipping date, blocks ambiguity and distinguishes markup from target margin',()=>{
  ...
  expect(calculateFclEstimate(data,{...estimateRequest(),shipping_date:'2027-01-01'},cosco,'calgary').blockers).toContain('delivery_unavailable:van-calgary');
...
it('retains legacy versions for review and rejects duplicate references and invalid capacity',async()=>{
...
it('bills per container and holds ambiguous historical charge versions for review',()=>{
  ...
  expect(first.blockers).toContain('charge_ambiguous:thc');
  expect(later.blockers).toContain('charge_ambiguous:thc');
  expect(first.totals.cost_total).toBeNull();
  expect(later.totals.cost_total).toBeNull();
});
