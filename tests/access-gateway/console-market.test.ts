...
describe('public capability market', () => {
  it('keeps the actual MCP boundary separate from the five business API capabilities', () => {
    ...
    expect(marketServices.filter(item=>item.protocol==='workspace').map(item=>item.id)).toEqual(['quote.documents','ocean.schedules','port.efficiency']);
    expect(new Set(marketServices.map((item: { id: string }) => item.id)).size).toBe(11);
  });
