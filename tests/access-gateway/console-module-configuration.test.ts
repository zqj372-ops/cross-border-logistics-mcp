...
it('uses only market modules for configuration and preserves legacy destinations',async()=>{
 ...
 expect(m.marketServices).toHaveLength(11);
 ...
 expect(m.configurationRoute('#configure/quote.documents').kind).toBe('quote-documents');
 expect(m.configurationRoute('#configure/ocean.schedules').kind).toBe('sailing-schedules');
 expect(m.configurationRoute('#configure/port.efficiency').kind).toBe('terminal-efficiency');
