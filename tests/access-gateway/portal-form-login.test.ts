...
it('validates credentials and one-use session-bound captcha, expires challenges and limits failures',()=>{
 ...
 expect(()=>login.challenge('new-session','127.0.0.2')).toThrow('login_rate_limited');
  ...
  const fcl=login.challenge('session-fcl','127.0.0.3');expect(login.verify('session-fcl','127.0.0.3',{account:'fcl-receiver@example.test',password:'FreightClaw2026!',captcha_id:fcl.captcha_id,captcha:'23456'})).toBe('fixture-fcl-receiver');
});
