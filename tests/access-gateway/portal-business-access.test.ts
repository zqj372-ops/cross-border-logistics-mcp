...
describe("business access v2 domain",()=>{
  it("issues a T0-only application Key, exchanges it through the restricted signer, and revokes old-token authority immediately", async () => {
    ...
    const decision = x.portal.decideRequest(ctx(x.reviewer, null), request.requestId, { idempotencyKey: "t0-request-review01", expectedVersion: 2, input: { decision: "approve", reason: "ok" } }).data! as {grant:{grantId:string}};
    x.portal.markGrantActive(ctx(x.operator, null), decision.grant.grantId, { idempotencyKey: "t0-grant-active001", expectedVersion: 1, input: { provisionedRef: "client:client_business" } });
