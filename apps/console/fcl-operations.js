...
export function createFclOperations({call:readRequest,write:writeRequest,esc,rerender,notify,model,api}){
  ...
  const matchingTemplates=()=>publishedOptions().templates.filter(t=>t.enabled&&validOn(t)&&(!routePod||t.pod===routePod)&&(!routeDestination||t.destination===routeDestination));
  const matchingRates=()=>(publishedDataset()?.rates||[]).filter(r=>validOn(r)&&(!query.pol||r.pol===query.pol)&&(!routePod||r.pod===routePod));
  const queryForm=()=>{
  ...
  const oceanAction=async button=>{
    ...
    if(name==='ops-ocean-sailing'){schedules.show(r,query.shipping_date||r.valid_from,draftOptions().rate_details.find(d=>d.rate_id===r.rate_id));return true;}
    ...
    if(name==='ops-ocean-disable'){if(draft.rates.length===1){message='最后一条海运费请通过高级设置中的“停用当前运价发布”停用。';rerender();return true;}if(!window.confirm('将此条海运费从下一次发布中停用？历史版本仍保留。'))return true;draft.rates.splice(i,1);draft.operations.rate_details=draftOptions().rate_details.filter(d=>d.rate_id!==r.rate_id);schedules.invalidate();}
  ...
  const submit=async form=>{
    ...
    if(kind==='ops-run'){
      ...
      const response=await write('estimate-run',query),result=requireData(response);await refresh();
