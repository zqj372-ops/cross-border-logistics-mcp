...
<div class="field"><label for="live-routing">直达 / 中转</label><select id="live-routing" name="routing"><option value="any" ${live.routing==='any'?'selected':''}>不限</option><option value="direct" ${live.routing==='direct'?'selected':''}>直达</option><option value="transshipment" ${live.routing==='transshipment'?'selected':''}>中转</option></select></div>
  ...
  function liveRecord(record){
    ...
    return `<article class="schedule-card"><div class="voyage-grid"><div class="voyage-vessel"><span class="voyage-label">船名 / 航次</span><strong>${esc(vessel||'船名未提供')}</strong><span>${voyage?'航次 '+esc(voyage):'航次未提供'}</span></div><div><span class="voyage-label">码头计划</span><strong>${esc(record.terminal?.name||'未提供')}</strong></div><div><span class="voyage-label">计划开航</span>${timeCell(planned,'离港时间未提供')}<small>${esc(first?.from?.name||'起运地未提供')}</small></div><div><span class="voyage-label">实际离港</span>${timeCell(actual,'未提供')}</div><div><span class="voyage-label">目的地到达</span>${timeCell(arrival,'到达时间未提供')}<small>${esc(last?.to?.name||'目的地未提供')}</small></div></div><details class="schedule-voyage-details"><summary>航次详情</summary><div class="schedule-detail-body"><h3>运输路线 · ${routing}</h3><div class="schedule-itinerary">${legs}</div></div></details></article>`;
  ...
  function weeklyOverview(records,partial){
    ...
    return `<div class="schedule-section-heading service-overview-heading"><h3>航线周览</h3></div>${overview('direct','直达')}${overview('transshipment','中转')}`;
  }
