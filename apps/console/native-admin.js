...
export function createNativeAdminUi({ api, mutate, esc, head, note, icon, formError, model, rerender, canConfigure }) {
  ...
  function ratesEditor(section) {
    const d = editor;
    let body = '';
    if (section === 'base') body = `<h2>基本资料</h2><p>私人地址派送使用一套固定业务配置，无需选择渠道。</p><div class="field-grid">${field('运价名称', 'label', d.label)}${field('当前起运地', 'origin', d.origin)}${field('开始日期', 'valid_from', d.valid_from, 'date')}${field('截止日期', 'valid_until', d.valid_until, 'date')}${field('来源文件或协议编号', 'evidence_ref', d.evidence_ref)}${field('来源版本', 'evidence_version', d.evidence_version)}${field('报价单有效天数', 'extensions.quote_valid_days_v1', d.extensions?.quote_valid_days_v1, 'number', '沿用来源约定；不得超过运价截止日期。')}</div><div class="field"><label for="native-customer_terms">客户适用条件</label><textarea id="native-customer_terms" name="customer_terms" rows="5" maxlength="2000">${esc(d.customer_terms)}</textarea><small>说明交付范围、卸货、预约和有效条件。</small></div><p class="muted">自有运价按 USD 维护，Freightcom 保留承运商返回币种。</p>`;
