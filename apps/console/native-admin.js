...
const names = { 'customs-data': '关务管理', 'residential-rates': '私人地址运价', freightcom: '外部连接' };
const rateSections = { base: '基本资料', coverage: '邮编覆盖', pricing: '价格与计费', fees: '附加费', publish: '核验与发布' };
const customsSections = { nomenclature: '税号目录', tariffs: '税率', measures: '贸易措施', requirements: '单证要求', sources: '来源', import: '导入数据', publish: '核验与发布' };
...
export function createNativeAdminUi({ api, mutate, esc, head, note, icon, formError, model, rerender, canConfigure }) {
  ...
  function ratesEditor(section) {
    const d = editor;
    let body = '';
    if (section === 'base') body = `<h2>基本资料</h2><p>私人地址派送使用一套固定业务配置，无需选择渠道。</p><div class="field-grid">${field('运价名称', 'label', d.label)}${field('固定始发仓', 'origin', d.origin)}${field('开始日期', 'valid_from', d.valid_from, 'date')}${field('截止日期', 'valid_until', d.valid_until, 'date')}${field('来源文件或协议编号', 'evidence_ref', d.evidence_ref)}${field('来源版本', 'evidence_version', d.evidence_version)}</div><div class="field"><label for="native-customer_terms">客户适用条件</label><textarea id="native-customer_terms" name="customer_terms" rows="5" maxlength="2000">${esc(d.customer_terms)}</textarea><small>说明交付范围、卸货、预约和有效条件。</small></div><p class="muted">自有运价按 USD 维护，Freightcom 保留承运商返回币种。</p>`;
    if (section === 'coverage') body = `<h2>邮编覆盖</h2><p>完整邮编优先于前三位；相同邮编只能配置一次。</p>${rateEditor.filterBar('zones')}${editTable('zones', [['postal_prefix', '邮编 / 前三位'], ['zone', '分区编号'], ['city', '城市'], ['province', '省份代码']])}`;
    if (section === 'pricing') body = `${rateEditor.matrix()}<details class="native-row-editor" ${d.rates.length===0||d.rates.some(r=>r.zone===''||r.pallets==='')?'open':''}><summary>逐条新增或删除价格</summary>${editTable('rates', [['zone', '分区编号'], ['pallets', '计费托数'], ['amount', '基础价 · USD']])}</details><details class="native-row-editor native-billing-editor"><summary>计费规则与货物范围</summary><div class="field-grid">${Object.entries(billingLabels).map(([key, label]) => field(label, 'billing.' + key, d.billing[key])).join('')}</div></details>`;
  ...
  function input(event) {
    ...
    const last = parts.at(-1), text = event.target.value.trim(); target[last] = integerKeys.has(last) && text !== '' ? Number(text) : text;
  ...
  async function submit(form) {
    ...
    if (kind === 'residential-rates') {
      const normalized = structuredClone(editor); normalized.zones = normalized.zones.map(row => ({ ...row, postal_prefix: String(row.postal_prefix || '').replaceAll(' ', '').toUpperCase(), province: String(row.province || '').toUpperCase() }));
