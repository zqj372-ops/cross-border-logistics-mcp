...
export function createCasesUi({ api, mutate, head, panel, empty, esc, date, icon, formError, rerender, model }) {
  let generation = 0, scope = '', cache = new Map(), filter = '', cursor = '';
  function reset() { generation++; cache.clear(); filter = ''; cursor = ''; }
  ...
  function load(path) {
    ...
    if (!cache.has(path)) {
      ...
      api(path).then(result => { if (epoch === generation) record.data = result.data; }).catch(error => { if (epoch === generation) record.error = error; }).finally(() => { if (epoch === generation) { record.loading = false; rerender(); } });
    }
  ...
  function detail(id) {
    ...
    const item = record.data, d = item.input;
    ...
    return `<a class="case-back" href="#${item.can_manage ? 'operations' : 'cases'}">${icon('back')}返回询价列表</a>` + head(d.product || '企业与合规服务', `需求编号 ${item.case_id}`, refresh) + `<div class="case-status-line">${label(item.status)}<span>创建于 ${date(item.created_at)}</span><span>费用待报价确认</span></div><div class="case-workspace"><div>${info}</div><div>${form}${timeline}</div></div>`;
  }
  ...
  return { page, detail, action, submit, reset };
}
