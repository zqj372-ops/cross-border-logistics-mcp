...
## Market module configuration refinement · 2026-09-08

The service market is the sole configuration module directory. `service-catalog.js` owns matching names, icons, categories and a narrow configuration mapping; public usage and administrator configuration views share it. Configurable modules lead the configuration list, while modules without enterprise settings or without an implemented editor state that fact. Both customs capabilities point to the same existing customs configuration. Fixed residential rates belong to Canada final-mile quoting and Freightcom belongs to carrier LTL quoting.

`#market/configure` is the directory and `#configure/<market-service-id>/<section>` is the editor. Legacy business-admin routes normalize to these destinations. The old three-row business administration overview and its styling were removed. Account/workspace links point to the same market configuration view. On configuration pages show the current organization and current module tabs, omitting the personal-center navigation row. Preserve draft edits when returning through the market. Editors retain original save/preview/publish and CLI contracts; route mapping grants no server authority.

This narrow Operate/distill refinement preserves the incumbent palette, fonts, native controls and local overflow. No new visual world, generated assets or global tokens. Local checks cover desktop, tablet and phone plus directory/role/legacy-route and prior native business workflows. Earlier finish-review dispositions remain scoped to their earlier changes; no new independent reviewer disposition is claimed for this refinement.
