> 配置入口已统一到「服务市场 → 模块配置」。私人地址报价 · 自有运价对应固定运价，私人地址报价 · Freightcom 对应企业承运商连接，两个关务模块共用同一份关务配置。旧业务管理链接继续兼容跳转；本文的保存、核验、发布和 CLI 操作保持有效。

# 关务与私人地址业务后台

后台入口：服务市场 → 模块配置。当前企业的负责人、管理员可修改配置；普通成员只可查询。平台角色不代替企业身份修改此处配置。私人地址运价、关务及 Freightcom 分别从对应市场模块进入，与 CLI 共用接口。
...
使用当前产品版本 `0.008` 构建的 CLI；生产配置与网页部署完成后再从生产地址使用新操作。CLI 通过浏览器确认获得独立人员会话；现有查询 API Key 不增加后台管理权限。
...
当前支持文本和粘贴的制表符／竖线表格。表头须声明件数、尺寸及重量单位，重量需注明单重或总重。不接收图片／PDF，不做地址地图验证，也不自动保存正式报价、审核或发邮件。原生试算成功后可制作报价单，由服务器重算绑定来源，完成保存、人员审核或退回以及 PDF 导出。


## 完整法规更新与两地运价

关务模块新增「数据更新」，接受维护人员已传入接收目录的完整 SQLite 快照。数据包按哈希校验并保留来源审核状态；未就绪版本可在目录选择「已保存草稿」查看，不能启用。目录网页和 `customs-packages browse` 共用完整 SQL 数据。手动 JSON 补录保留 16 MiB 限额；其发布不替换已选中的完整数据包。

私人地址仍是固定服务；Toronto、Calgary 作为不同起运地维护各自的邮编、价格矩阵、燃油及分区开关。共同条款、附加费和有效期只配置一份。起运地通过 `extensions.origins_v1` 保存，询价用 `extensions.origin_v1` 明确选择；邮编只能匹配一个起运地时可留空。最大重量、体积、长度允许 null，含义是来源没有设置该项上限，并非免除人工复核。

报价单有效天数通过 `extensions.quote_valid_days_v1` 配置，且不得晚于运价截止。当前本地迁入的线上价格截止 2027-01-01，有效天数 7；冲突邮编未自动分配。完整更新命令及原生报价保存/审核/退回见 [人员 CLI](workspace-cli.md)。当前部署状态以仓库 README 和本次运行手册为准。

城市分区模式支持同一邮编覆盖不同城市；在邮编覆盖页开启后，网页和 CLI 使用相同的城市匹配规则。缺城市要求补填，同城多分区保持人工复核，CSV 导入保留冲突供核对。


## 独立报价页面（2026-09-08）

自有运价从 `#quote/private` 进入，Freightcom 从 `#quote/freightcom` 进入。各自维护资料和结果；网页不再自动共用地址、卸货条件或货物。Freightcom 每行是一个实体托盘，必须分别填写重量、尺寸、托盘内件数、描述和货运等级。

CLI 保持 `workspace quote self` 与 `workspace quote freightcom` 两条独立查询。旧 `workspace quote shared-preview` 仅保留显式转换的兼容用途，不会查询承运商，不在新页面自动执行。

## 船期和码头效率

从市场的船期查询、码头效率分别进入配置。先填写来源、观察时间、有效期和记录，保存草稿，预览后确认发布。全部操作也可使用 `workspace schedules` 和 `workspace terminals`。无数据或过期时前台明确提示；当前没有自动同步船司或港务局 API。详见 [人员 CLI](workspace-cli.md)。
