...
本文按当前实现合并更新字体、阅读尺度、布局与图标规范，保留可见焦点、局部滚动、移动账号入口与状态文字。视觉依据为当前 `styles.css`、`home.js`、`icons.js`、字体清单和 `.impeccable/review/refinement` 中的本地桌面、移动端截图。生产部署与业务就绪状态归交付记录和 [PRODUCT.md](PRODUCT.md)，不由设计文档推定。
