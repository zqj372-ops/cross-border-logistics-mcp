...
export const cliDownload = '/downloads/freightclaw-cli-0.003.tgz';
...
export function createCliGuide(ui) {
  ...
  function page() {
    return `<div class="cli-page"><div class="cli-intro"><div><p class="entry-eyebrow">FREIGHTCLAW CLI <span>v0.003</span></p><h1>把物流能力，<br>带进你的终端。</h1><p>询价、关税、货物计算。使用同一把 API Key，在本机、脚本或 Agent 中调用已开通的服务。</p><div class="entry-actions"><a class="button primary" href="${cliDownload}" download>下载安装包 ${icon('arrow')}</a>${link('管理 API Key', 'api-keys')}</div><p class="cli-requirement">macOS / Windows / Linux · 需要 Node.js 22.13 或更新版本</p></div><div class="cli-terminal" aria-label="命令示例"><div class="cli-terminal-bar"><span>FreightClaw / Terminal</span><span>JSON 输出</span></div><pre><span class="terminal-comment"># 检查连接，无需 Key</span>\n$ freightclaw status\n\n<span class="terminal-comment"># 查看关税查询的输入要求</span>\n$ freightclaw schema customs query\n\n<span class="terminal-comment"># 选择业务，传入 JSON 文件</span>\n$ freightclaw customs query \\\n    --input ./customs-query.json --json</pre><p>命令示例；实际结果以服务返回为准。</p></div></div>
    <section class="panel workspace-cli-guide"><div class="panel-body"><h2>workspace 人员与 FCL 工作流</h2><p>渠道配置、询价处理、FCL 公开询价与个人报价工作流使用同一服务端记录和权限。public inquiry 无需登录；staff person 操作先完成 device flow，API Key 不能替代本人身份。</p><p>运行 <code>workspace --help</code> 查看步骤，执行 <code>workspace commands</code> 查看当前版本支持范围。</p><div class="channel-actions">${link('渠道与仓库', 'channels')}<a class="button" href="/console/workspace-cli.md#fcl-人员与公开询价" target="_blank" rel="noopener">FCL workspace 说明 ${icon('arrow')}</a></div></div></section><div class="cli-body"><article><section class="cli-step"><div class="cli-step-title"><span>1</span><h2>安装并检查连接</h2></div><p>复制下方命令安装，或下载上方安装包后使用 npm 安装本地文件。当前通过本站提供安装包，未发布到公共 npm registry。</p>${block(installCommand, 'install')}${block('freightclaw --version\nfreightclaw status', 'status')}<p class="muted">连接检查只说明门户状态；每次业务调用仍会检查服务授权与来源数据。</p><a class="text-button" href="/downloads/freightclaw-cli-0.003.sha256" download>下载安装包校验值</a></section>
    ...
    <aside class="cli-aside"><h2>接着了解</h2><a href="https://github.com/zqj372-ops/cross-border-logistics-mcp/blob/main/docs/runbooks/freightclaw-cli-illustrated.md" target="_blank" rel="noopener">图文使用指南 ${icon('arrow')}</a><a href="https://github.com/zqj372-ops/cross-border-logistics-mcp/blob/main/deploy/cli/README.md" target="_blank" rel="noopener">完整命令参考 ${icon('arrow')}</a><a href="/console/openapi.json" target="_blank" rel="noopener">OpenAPI 文档 ${icon('arrow')}</a>${link('操作手册', 'guide')}<div><h3>日常业务继续在线办理</h3><p>workspace 已覆盖 FCL 公开询价、个人案件、运价、报价、审核、PDF 与交接。人员操作使用 device flow；API Key 只用于机器接入。</p>${link('打开业务工作台', 'workbench')}</div></aside></div></div>`;
  }
  function example() {
    return `<div class="cli-example-links"><a class="button" href="/downloads/cli-0.003/examples/${selected.id}.json" download="${selected.id}.json">下载 ${selected.id}.json</a><span>合成输入示例，使用前请替换</span></div>${block(command(), 'business')}${block(`freightclaw schema ${selected.command}`, 'schema', '复制 Schema 命令')}`;
  }
