...
export function createFclWorkspace({api, mutate, model, esc, head, panel, empty, note, field, actions, formError, icon, rerender, notify}) {
  ...
  const detailPage = (id) => {
    ...
    const steps = [
      ['requirements', '确认需求', detail.review_context.review_required ? '待核对' : '已确认'],
