export const marketServices = Object.freeze([
  ...
  { id: 'quote.zone_preview', configuration: { kind:'residential-rates', description:'固定私人地址派送：邮编覆盖、价格矩阵、计费规则和附加费。' }, name: '私人地址报价 · 自有运价', category: 'quote', protocol: 'api', icon: 'truck', tone: 'blue', provider: 'FreightClaw 询价服务', description: '按邮编、货物与卸货条件试算尾程运费，查看分区和费用依据。', detail: '使用当前企业发布的邮编、城市分区与自有运价。独立填写货物和派送条件，试算后可制作、保存、审核并导出报价单。', outputs: ['邮编匹配与服务分区', '价格试算及费用构成', '缺失条件与人工复核提示'], path: '/api/v2/business/quote/zone-preview', online: 'quote/private' },
  { id: 'quote.ai_extract_preview', configuration: { state:'fixed', description:'原生文字解析随服务版本发布，无需模型密钥；缺失和冲突资料由询价页面确认。' }, name: '询价资料提取', category: 'quote', protocol: 'api', icon: 'file', tone: 'violet', provider: 'FreightClaw 询价服务', description: '把客户的询价描述整理为结构化资料，补齐询价前的关键条件。', detail: '提取邮编、数量、体积、重量和交付条件，并保留需要核对的内容。提取结果不代替计价规则，也不自动确认客户条件。', outputs: ['结构化货物与地址信息', '待补充的业务字段', '可继续试算的询价资料'], path: '/api/v2/business/quote/ai-extract-preview', online: 'quote/private' },
  ...
  { id: 'quote.freightcom_ltl.preview', configuration: { kind:'freightcom', description:'维护当前企业的 Freightcom 连接，并通过实际询价核对。' }, name: '私人地址报价 · Freightcom', category: 'quote', protocol: 'api', icon: 'truck', tone: 'orange', provider: 'Freightcom 适配服务', description: '查询承运商提供的 LTL 方案，核对运价与运输条件。', detail: '使用企业配置的 Freightcom 连接获取只读询价结果。需要有效承运商凭证和完整货物、地址资料，查询本身不提交订舱。', outputs: ['承运商询价结果', '服务条件与报价有效性', '连接或输入问题'], path: '/api/v2/business/quote/freightcom-ltl-preview', online: 'quote/freightcom' },
  {id:'ocean.schedules',configuration:{kind:'sailing-schedules',description:'按模块维护船期来源、航次与有效期，核验后发布到企业工作区。'},name:'船期查询',category:'ocean',protocol:'workspace',icon:'ship',tone:'blue',provider:'船司与港务局来源',description:'查询中国到加拿大船期，核对航次、预计到离港与中转安排。',detail:'公开查看船司和港务局入口。企业成员可查询本企业核验发布的船期快照；计划、预计、实际分别标注，保留来源时区。当前没有自动同步船司 API。',outputs:['航次与直达 / 中转安排','各自时区的计划、预计或实际到离港','来源版本、观察时间与有效期'],path:'/console/api/v1/maritime/sailing-schedules/query',online:'schedules'},
  {id:'port.efficiency',configuration:{kind:'terminal-efficiency',description:'维护港口、码头、指标、统计区间及来源，分别核验并发布。'},name:'码头效率',category:'ocean',protocol:'workspace',icon:'clock',tone:'cyan',provider:'港务局与交通部门来源',description:'查看铁路滞箱、锚地等待、闸口周转和堆场占用情况。',detail:'按港口、码头和统计口径读取本企业发布的运营快照，也可打开官方报告。未发布或已过期时明确提示；不将缺失值当作 0，不合成无来源的拥堵分数。当前没有自动同步上游指标。',outputs:['按港口和码头分开的效率指标','数值、单位、统计窗口与口径','资料缺失、过期和来源提示'],path:'/console/api/v1/maritime/terminal-efficiency/query',online:'terminal-efficiency'},
]);
...
export function agentInstallPrompt(serviceId = '') {
  ...
  if(['ocean.schedules','port.efficiency'].includes(serviceId))return `阅读 https://www.freightclaw.net/console/workspace-cli.md，通过 workspace ${serviceId==='ocean.schedules'?'schedules':'terminals'} 命令查询和管理企业快照；先完成网页登录授权，核对来源有效期。`;
