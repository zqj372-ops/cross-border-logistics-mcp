...
function customerNav() {
  ...
  const consolePages = ['workbench', 'quote', 'quote-history', 'customs', 'tax', 'api-keys', 'apply', 'requests', 'request', 'request-new', 'request-edit', 'grants', 'grant-edit', 'app', 'app-new', 'applications', 'credential-new', 'business-request', 'business-request-new', 'business-request-edit', 'business-grant', 'business-credential', 'activity', 'members', 'member-new', 'member-edit'];
...
function nav() {
  ...
  else navigation = `<div class="nav-group">${item('工作概览', 'home', 'home')}<p class="nav-label">业务工作</p>${model.session.organization_id ? `${item('加拿大尾程询价', 'quote', 'truck')}${item('我的报价记录', 'quote-history', 'clock')}${item('关税与归类', 'customs', 'file')}${item('税费估算', 'tax', 'file')}` : ''}${item('服务目录', 'catalog', 'grid')}</div>${developer() ? `<div class="nav-group"><p class="nav-label">应用与接入</p>${item('我的应用', 'applications', 'code')}${item('API 申请', 'requests', 'file', pending)}${item('服务授权', 'grants', 'shield')}${item('接入指引', 'guide', 'key')}</div>` : ''}<div class="nav-group"><p class="nav-label">组织管理</p>${item('成员与邀请', 'members', 'users')}${model.session.organization_id ? item('操作记录', 'activity', 'clock') : ''}</div>`;
...
async function runAction(button) {
  ...
  if (action === 'logout') { closeSecret(); model.verificationAbort?.abort(); model.verification = null; business.reset(); tax.reset(); developerGuide.reset(); businessAccess.reset(); serviceAccess.reset(); model.credentials.clear(); model.state = null; model.session = await mutate('/logout', 'POST', {}); model.requestKeys.clear(); render(); return; }
...
document.addEventListener('change', async (event) => {
  ...
  try { model.session = await mutate('/session/organization', 'POST', { organization_id: event.target.value || null }); closeSecret(); business.reset(); tax.reset(); developerGuide.reset(); businessAccess.reset(); serviceAccess.reset(); model.verificationAbort?.abort(); model.verification = null; await refresh(); go('home'); render(); } catch (error) { notify(errorMessage(error), true); render(); }
});
