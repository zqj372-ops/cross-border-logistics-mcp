...
function renderLogin() {
  rememberLoginDestination(['case','channels','cli-authorize'].includes(route().page) && route().id ? `${route().page}/${route().id}` : route().page, loginStorage());
...
function customerNav() {
  ...
  const consolePages = ['channels', 'cli-authorize', 'cases', 'operations', 'case', 'platform', 'organizations', 'organization', 'organization-new', 'workbench', 'customs-history', 'calls', 'quote', 'quote-history', 'account', 'api-keys', 'apply', 'requests', 'request', 'request-new', 'request-edit', 'grants', 'grant-edit', 'app', 'app-new', 'applications', 'credential-new', 'business-request', 'business-request-new', 'business-request-edit', 'business-grant', 'business-credential', 'activity', 'members', 'member-new', 'member-edit'];
  const subTarget = page === 'case' ? (reviewer() || manager() ? 'operations' : 'cases') : page === 'account' ? center : ['api-keys', 'credential-new', 'business-credential'].includes(page) ? 'api-keys' : ['apply', 'requests', 'request', 'request-new', 'request-edit', 'grants', 'grant-edit', 'business-request', 'business-request-new', 'business-request-edit', 'business-grant'].includes(page) ? 'apply' : page.startsWith('member') ? 'members' : page.startsWith('app') ? 'applications' : page;
  const subnav = signedIn && consolePages.includes(page) ? `<div class="console-context"><div class="console-organization">${organization}</div><nav aria-label="个人中心导航">${(reviewer() ? [...(model.session.identity.platform_role === 'operator' ? [['询价管理', 'operations'], ['业务管理', 'channels']] : []), ['工作概览', 'platform'], ['申请审核', 'requests'], ...(model.session.identity.platform_role === 'operator' ? [['服务授权', 'grants'], ['企业准入', 'organizations']] : [])] : [['我的询价', 'cases'], ...(manager() ? [['询价管理', 'operations'], ['业务管理', 'channels']] : []), ['工作台', 'workbench'], ...(developer() ? [['API Key', 'api-keys'], ['已开通服务', 'apply']] : []), [manager() ? '成员管理' : '企业成员', 'members'], ['关务历史', 'customs-history'], ['调用记录', 'calls'], ['操作记录', 'activity']]).map(([label, target]) => `<button type="button" class="console-tab" data-go="${target}" ${target === subTarget ? 'aria-current="page"' : ''}>${label}</button>`).join('')}</nav></div>` : '';
...
async function runAction(button) {
  ...
  if (action === 'logout') { model.sessionGeneration = (model.sessionGeneration || 0) + 1; consumeLoginDestination(loginStorage()); closeSecret(); model.verificationAbort?.abort(); model.verification = null; channels.reset(); cases.reset(); business.reset(); tax.reset(); customsHistory.reset(); calls.reset(); developerGuide.reset(); businessAccess.reset(); serviceAccess.reset(); model.credentials.clear(); model.state = null; model.session = await mutate('/logout', 'POST', {}); model.requestKeys.clear(); model.directory = null; model.businessCatalog = []; go('home'); render(); return; }
