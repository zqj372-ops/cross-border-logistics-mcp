...
import { createFclWorkspace } from "./fcl.js";
...
function platformIdentity() { return ['reviewer', 'operator'].includes(model.session?.identity?.platform_role); }
function reviewer() { return platformIdentity() && !model.session?.organization_id; }
function operator() { return reviewer() && model.session?.identity?.platform_role === 'operator'; }
function manager() { return ['owner', 'admin'].includes(orgRole()); }
function developer() { return ['owner', 'admin', 'developer'].includes(orgRole()); }
function canCredential(application) { return !platformIdentity() && developer() && application?.owner_user_id === model.session?.identity?.user_id; }
...
const errors = {
  ...
  fcl_personal_workspace_switch_failed: '整柜业务工作区没有切换成功，请重试。',
...
const organizationSession = () => !!(model.session?.authenticated && model.session.organization_id && orgRole());
let sessionLoading;
...
async function refresh() {
  ...
  if (!model.session.organization_id && !platformIdentity() && model.session.fcl_capability?.fcl_personal !== true) {
...
function customerNav() {
  ...
  const workspaces = [...((platformIdentity()||fclPersonal) ? [{ organization_id: '', display_name: fclPersonal ? '整柜业务工作区' : '平台工作区' }] : []), ...orgs];
  const selectableWorkspaces = workspaces.filter((item) => item.organization_id);
  const organizationPicker = workspaces.length > 1 || (page === 'schedules' && selectableWorkspaces.length > 0 && !model.session.organization_id);
  const organization = organizationPicker ? `<label for="organization">当前工作区</label><select id="organization" class="organization-select">${workspaces.map((item) => `<option value="${esc(item.organization_id)}" ${String(model.session.organization_id || '') === item.organization_id ? 'selected' : ''}>${esc(item.display_name)}</option>`).join('')}</select>` : `<span>${esc(workspaces[0]?.display_name || (signedIn ? model.session.identity.display_name : ''))}</span>`;
  const consolePages = ['business-admin', 'channels', 'cli-authorize', 'cases', 'operations', 'case', 'platform', 'organizations', 'organization', 'organization-new', 'workbench', 'customs-history', 'calls', 'quote', 'quote-history', 'account', 'api-keys', 'apply', 'requests', 'request', 'request-new', 'request-edit', 'grants', 'grant-edit', 'app', 'app-new', 'applications', 'credential-new', 'business-request', 'business-request-new', 'business-request-edit', 'business-grant', 'business-credential', 'activity', 'members', 'member-new', 'member-edit', ...(fclPersonal ? ['fcl'] : [])];
  ...
  const enterpriseDeveloperTabs = developer() ? [...(!platformIdentity() ? [['API Key', 'api-keys']] : []), ['已开通服务', 'apply']] : [];
  const subnav = signedIn && page === 'fcl' ? `<div class="console-context"><div class="console-organization">整柜业务工作区</div><nav aria-label="整柜业务导航">${[['询价列表','fcl'],['整柜报价工作台','fcl/compare'],['报价与通知设置','fcl/config']].map(([label,target]) => `<button type="button" class="console-tab" data-go="${target}"${(target === 'fcl' ? !['rates','templates','config','compare'].includes(route().id) : target === `fcl/${['rates','templates'].includes(route().id)?'compare':route().id}`) ? ' aria-current="page"' : ''}>${label}</button>`).join('')}</nav></div>` : signedIn && configurationPage ? `<div class="console-context module-context"><div class="console-organization">${organization}</div></div>` : signedIn && (consolePages.includes(page) || page === 'configure' || page === 'market' && route().id === 'configure') ? `<div class="console-context"><div class="console-organization">${organization}</div><nav aria-label="个人中心导航">${(reviewer() ? [...(operator() ? [['询价管理', 'operations']] : []), ...(fclPersonal ? [['整柜询价', 'fcl']] : []), ['工作概览', 'platform'], ['申请审核', 'requests'], ...(operator() ? [['服务授权', 'grants'], ['企业准入', 'organizations']] : [])] : [['我的询价', 'cases'], ...(fclPersonal ? [['整柜询价', 'fcl']] : []), ...(manager() ? [['询价管理', 'operations'], ['模块配置', 'market/configure']] : []), ['工作台', 'workbench'], ...enterpriseDeveloperTabs, [manager() ? '成员管理' : '企业成员', 'members'], ['关务历史', 'customs-history'], ['调用记录', 'calls'], ['操作记录', 'activity']]).map(([label, target]) => `<button type="button" class="console-tab" data-go="${target}" ${target === subTarget ? 'aria-current="page"' : ''}>${label}</button>`).join('')}</nav></div>` : signedIn && page === 'schedules' ? `<div class="console-context module-context"><div class="console-organization">${organization}</div></div>` : '';
...
function dashboard() {
  ...
  if (!s.current_organization && !reviewer()) tasks.push(['users', '加入企业，开始协作', '接受与你登录邮箱匹配的企业邀请。', '查看邀请', 'members']);
    ...
    if (!tasks.length) tasks.push(operator() ? ['users', '完善企业准入', '核对企业资料和负责人邀请，完成后由企业成员提交服务申请。', '进入企业准入', 'organizations'] : ['file', '等待企业提交申请', '企业负责人创建应用并提交服务申请后，审核任务会出现在这里。', '查看申请', 'requests']);
  }
  else {
...
function grantsPage() {
  ...
  return head('服务授权', '查看批准范围与有效期。停用、撤销或缩小范围会影响应用调用。') + `<section class="panel">${items.length ? table(['应用 / 服务', '授权状态', '有效期', '操作'], items.map((v) => `<tr><td><span class="cell-title">${esc(appName(v.application_id))}</span><span class="cell-detail">${capPills(v.capabilities)}</span></td><td>${badge(v.state)}</td><td>${v.expires_at ? esc(date(v.expires_at)) : '未设置截止时间'}</td><td><div class="cell-actions">${operator() && v.state === 'provisioning' ? `<button class="text-button" data-action="provision" data-id="${esc(v.grant_id)}">核对并开通</button>` : ''}${(operator() || manager()) && ['active', 'suspended'].includes(v.state) ? `${textLink('管理授权', `grant-edit/${v.grant_id}`)}` : ''}${textLink('查看申请', `request/${v.request_id}`)}</div></td></tr>`)) : empty('暂无服务授权', 'API 申请通过后，这里会出现批准的范围与开通进度。', link('查看申请', 'requests'), 'shield')}</section>`;
}
...
async function submitForm(form) {
  ...
  if (await fcl.submit(form)) return;
...
function resetWorkspaceViews(stayOnSchedules = false) {
  closeSecret();
  channels.reset();
  nativeAdmin.reset();
  quoteDocuments.reset();
  fcl.reset();
  maritime.reset(stayOnSchedules);
  workspaceHome.reset();
  cases.reset();
  business.reset();
  tax.reset();
  customsHistory.reset();
  calls.reset();
  developerGuide.reset();
  businessAccess.reset();
  serviceAccess.reset();
  model.verificationAbort?.abort();
  model.verification = null;
}

async function switchOrganization(organizationId, { stayOnSchedules = false, onSessionSelected } = {}) {
  const nextGeneration = (model.sessionGeneration || 0) + 1;
  model.sessionGeneration = nextGeneration;
  const session = await mutate('/session/organization', 'POST', { organization_id: organizationId });
  if ((model.sessionGeneration || 0) !== nextGeneration) throw Object.assign(new Error('account_changed'), { code: 'account_changed' });
  model.session = session;
  resetWorkspaceViews(stayOnSchedules);
  onSessionSelected?.();
  await refresh();
...
document.addEventListener('submit', async (event) => {
  const form = event.target.closest('form[data-form],form[data-fcl-form]'); if (!form) return; event.preventDefault();
  if (form.dataset.form === 'qdoc-editor') return;
...
document.addEventListener('change', async (event) => {
  ...
  try {
    ...
    go(stayOnSchedules ? 'schedules' : (model.session.organization_id ? 'workbench' : model.session.fcl_capability?.fcl_personal ? 'fcl' : 'platform'));
    render();
  } catch (error) {
...
document.addEventListener('toggle', (event) => { fcl.toggle(event); }, true);
...
window.addEventListener('beforeunload', event => { if (nativeAdmin.isDirty() || quoteDocuments.isDirty() || fcl.isDirty() || maritime.isDirty()) { event.preventDefault(); event.returnValue = ''; } });
