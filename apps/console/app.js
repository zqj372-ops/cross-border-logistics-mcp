...
function customerNav() {
  ...
  const subnav = signedIn && consolePages.includes(page) ? `<div class="console-context"><div class="console-organization">${organization}</div><nav aria-label="个人中心导航">${(reviewer() ? [...(model.session.identity.platform_role === 'operator' ? [['询价管理', 'operations']] : []), ['工作概览', 'platform'], ['申请审核', 'requests'], ...(model.session.identity.platform_role === 'operator' ? [['服务授权', 'grants'], ['企业准入', 'organizations']] : [])] : [['我的询价', 'cases'], ...(manager() ? [['询价管理', 'operations']] : []), ['工作台', 'workbench'], ...(developer() ? [['API Key', 'api-keys'], ['已开通服务', 'apply']] : []), [manager() ? '成员管理' : '企业成员', 'members'], ['关务历史', 'customs-history'], ['调用记录', 'calls'], ['操作记录', 'activity']]).map(([label, target]) => `<button type="button" class="console-tab" data-go="${target}" ${target === subTarget ? 'aria-current="page"' : ''}>${label}</button>`).join('')}</nav></div>` : '';
