...
async function refresh() {
  ...
  if (!model.session.organization_id && !platformIdentity()) {
...
function customerNav() {
  ...
  const workspaces = [...(platformIdentity() ? [{ organization_id: '', display_name: '平台工作区' }] : []), ...orgs];
  ...
  const subnav = signedIn && configurationPage ? `<div class="console-context module-context"><div class="console-organization">${organization}</div></div>` : signedIn && (consolePages.includes(page) || page === 'configure' || page === 'market' && route().id === 'configure') ? `<div class="console-context"><div class="console-organization">${organization}</div><nav aria-label="个人中心导航">${(reviewer() ? [...(operator() ? [['询价管理', 'operations']] : []), ['工作概览', 'platform'], ['申请审核', 'requests'], ...(operator() ? [['服务授权', 'grants'], ['企业准入', 'organizations']] : [])] : [['我的询价', 'cases'], ...(manager() ? [['询价管理', 'operations'], ['模块配置', 'market/configure']] : []), ['工作台', 'workbench'], ...enterpriseDeveloperTabs, [manager() ? '成员管理' : '企业成员', 'members'], ['关务历史', 'customs-history'], ['调用记录', 'calls'], ['操作记录', 'activity']]).map(([label, target]) => `<button type="button" class="console-tab" data-go="${target}" ${target === subTarget ? 'aria-current="page"' : ''}>${label}</button>`).join('')}</nav></div>` : signedIn && page === 'schedules' ? `<div class="console-context module-context"><div class="console-organization">${organization}</div></div>` : '';
...
document.addEventListener('change', async (event) => {
  ...
  try { model.sessionGeneration = (model.sessionGeneration || 0) + 1; model.session = await mutate('/session/organization', 'POST', { organization_id: event.target.value || null }); closeSecret(); channels.reset(); nativeAdmin.reset(); quoteDocuments.reset(); maritime.reset(stayOnSchedules); workspaceHome.reset(); cases.reset(); business.reset(); tax.reset(); customsHistory.reset(); calls.reset(); developerGuide.reset(); businessAccess.reset(); serviceAccess.reset(); model.verificationAbort?.abort(); model.verification = null; await refresh(); go(stayOnSchedules ? 'schedules' : (model.session.organization_id ? 'workbench' : 'platform')); render(); } catch (error) { notify(errorMessage(error), true); render(); }
});
