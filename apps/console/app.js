...
function customerNav() {
  ...
  const subnav = signedIn && page === 'fcl' ? `<div class="console-context"><div class="console-organization">整柜业务工作区</div><nav aria-label="整柜业务导航">${[['询价列表','fcl'],['运价管理','fcl/rates'],['报价与通知设置','fcl/config']].map(([label,target]) => `<button type="button" class="console-tab" data-go="${target}"${(target === 'fcl' ? !['rates','config'].includes(route().id) : target === `fcl/${route().id}`) ? ' aria-current="page"' : ''}>${label}</button>`).join('')}</nav></div>` : signedIn && configurationPage ? `<div class="console-context module-context"><div class="console-organization">${organization}</div></div>` : signedIn && (consolePages.includes(page) || page === 'configure' || page === 'market' && route().id === 'configure') ? `<div class="console-context"><div class="console-organization">${organization}</div><nav aria-label="个人中心导航">${(reviewer() ? [...(operator() ? [['询价管理', 'operations']] : []), ...(fclPersonal ? [['整柜询价', 'fcl']] : []), ['工作概览', 'platform'], ['申请审核', 'requests'], ...(operator() ? [['服务授权', 'grants'], ['企业准入', 'organizations']] : [])] : [['我的询价', 'cases'], ...(fclPersonal ? [['整柜询价', 'fcl']] : []), ...(manager() ? [['询价管理', 'operations'], ['模块配置', 'market/configure']] : []), ['工作台', 'workbench'], ...enterpriseDeveloperTabs, [manager() ? '成员管理' : '企业成员', 'members'], ['关务历史', 'customs-history'], ['调用记录', 'calls'], ['操作记录', 'activity']]).map(([label, target]) => `<button type="button" class="console-tab" data-go="${target}" ${target === subTarget ? 'aria-current="page"' : ''}>${label}</button>`).join('')}</nav></div>` : signedIn && page === 'schedules' ? `<div class="console-context module-context"><div class="console-organization">${organization}</div></div>` : '';
