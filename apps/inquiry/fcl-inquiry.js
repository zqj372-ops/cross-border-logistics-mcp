...
function field(label, id, value, options = {}) {
  label=FCL_FIELDS[id]?fclField(id):fclFieldLabel(label);
...
function contactFields(draft, errors) {
  ...
  return `<h2 id="fcl-step-title" tabindex="-1">${steps[2]}</h2><p class="section-intro">请留下联系方式，工作人员核对需求后会与你联系并提供报价。</p>
    <div class="fcl-summary"><dl><dt>${fclField("origin_city")}</dt><dd>${esc(summary.route.origin_city || '待确认')}</dd><dt>${fclField("pol")}</dt><dd>${esc(summary.route.pol || '待确认')}</dd><dt>${fclField("pod")}</dt><dd>${esc(summary.route.pod || '待确认')}</dd><dt>${fclField("destination")}</dt><dd>${esc(summary.route.final_destination || '待确认')}</dd><dt>柜型</dt><dd>${esc(summary.containers.map(item => `${item.type} × ${item.quantity ?? '待确认'}`).join('、') || '待确认')}</dd><dt>${fclField("cargo_ready_date")}</dt><dd>${esc(summary.cargo.cargo_ready_date || '待确认')}</dd><dt>${fclField("incoterm")}</dt><dd>${esc(summary.cargo.incoterm || '待确认')}${summary.cargo.incoterm === 'Other' && summary.cargo.incoterm_other ? ` · ${esc(summary.cargo.incoterm_other)}` : ''}</dd><dt>服务</dt><dd>${esc(draft.selected_services.map(serviceLabel).join('、') || '待确认')}</dd></dl></div>
