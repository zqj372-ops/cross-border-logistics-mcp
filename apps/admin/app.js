...
export const TENANT_ACCESS_SCHEMA_VERSION = "2026-08-27.v1";
const TENANT_ACCESS_API_ROOT = "/admin/api/v1/access";
const TENANT_ACCESS_STATE_PATH = "/admin/api/v1/access/state";
const TENANT_ACCESS_TENANT_STATUSES = new Set([
  "active",
  "suspended",
]);
const TENANT_ACCESS_CREDENTIAL_STATUSES = new Set([
  "active",
  "expired",
  "revoked",
]);
const TENANT_ACCESS_DELIVERY_STATUSES = new Set(["pending", "acknowledged"]);
const TENANT_ACCESS_EFFECTIVE_STATUSES = new Set([
  "pending_delivery",
  "active",
  "tenant_suspended",
  "expired",
...
const TENANT_ACCESS_TENANT_ACTIONS = new Set(["activate", "suspend"]);
const TENANT_ACCESS_CREDENTIAL_ACTIONS = new Set([
  "acknowledge_delivery",
  "rotate",
  "revoke",
]);
const TENANT_ACCESS_OPERATION_ACTIONS = new Set([
  "tenant.create",
  "tenant.activate",
  "tenant.suspend",
  "credential.issue",
...
const TENANT_ACCESS_OPERATION_STATES = new Set([
  "absent",
  "active",
  "suspended",
  "pending_delivery",
...
const TENANT_ACCESS_OPERATION_TRANSITIONS = Object.freeze({
  "tenant.create": Object.freeze({ from: "absent", to: "active", credential: "none" }),
  "tenant.activate": Object.freeze({ from: "suspended", to: "active", credential: "none" }),
  "tenant.suspend": Object.freeze({ from: "active", to: "suspended", credential: "none" }),
  "credential.issue": Object.freeze({ from: "absent", to: "pending_delivery", credential: "required" }),
  "credential.delivery_acknowledge": Object.freeze({ from: "pending_delivery", to: "active", credential: "required" }),
  "credential.rotate": Object.freeze({ from: "active", to: "pending_delivery", credential: "required" }),
  "credential.revoke": Object.freeze({ from: ["pending_delivery", "active"], to: "revoked", credential: "required" }),
});
const TENANT_ACCESS_RESPONSE_STATUSES = new Set([
  "success",
  "needs_input",
  "manual_review",
  "blocked",
...
const TENANT_ACCESS_TOOL_NAMES = new Set([
  "cargo.calculate",
  "container.plan_summary",
  "system.agent_context.get",
]);
const TENANT_ACCESS_TENANT_KEYS = [
  "tenant_id",
  "display_name",
  "status",
  "created_at",
...
const TENANT_ACCESS_CREDENTIAL_KEYS = [
  "credential_id",
  "tenant_id",
  "client_id",
  "label",
...
const TENANT_ACCESS_OPERATION_KEYS = [
  "operation_id",
  "tenant_id",
  "credential_id",
  "actor_ref",
...
const TENANT_ACCESS_TOOL_CATALOG_KEYS = ["tool_name", "kind"];
const TENANT_ACCESS_STATE_KEYS = ["available_tools", "tenants", "credentials", "operations"];
const TENANT_ACCESS_ENVELOPE_KEYS = [
  "schema_version",
  "request_id",
  "trace_id",
  "audit_id",
...
const TENANT_ACCESS_IDENTIFIER_PATTERN = /^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/;
const TENANT_ACCESS_API_KEY_PATTERN = /^lmcpk_[A-Za-z0-9._:-]+_[A-Za-z0-9_-]{43}$/;
const TENANT_ACCESS_TOOL_LABELS = Object.freeze({
  "cargo.calculate": "货物与分泡计算",
  "container.plan_summary": "装柜方案汇总",
  "system.agent_context.get": "Agent 上下文",
});

function accessExactKeys(value, keys, label) {
...
function accessString(value, label) {
  if (typeof value !== "string" || value.trim() === "") throw new Error(`${label}必须是非空文本。`);
  return value;
}

function accessIdentifier(value, label) {
  accessString(value, label);
  if (!TENANT_ACCESS_IDENTIFIER_PATTERN.test(value)) throw new Error(`${label}格式无效。`);
  return value;
}

function accessNullableIdentifier(value, label) {
  if (value === null) return value;
  return accessIdentifier(value, label);
}

function accessTimestamp(value, label) {
  accessString(value, label);
  if (!Number.isFinite(Date.parse(value))) throw new Error(`${label}格式无效。`);
  return value;
}

function accessNullableTimestamp(value, label) {
  if (value === null) return value;
  return accessTimestamp(value, label);
}

function accessTenantRecord(value, index) {
  const label = `tenants[${index}]`;
  accessExactKeys(value, TENANT_ACCESS_TENANT_KEYS, label);
  accessIdentifier(value.tenant_id, `${label}.tenant_id`);
  accessString(value.display_name, `${label}.display_name`);
  if (!TENANT_ACCESS_TENANT_STATUSES.has(value.status)) {
    throw new Error(`${label}.status无效。`);
  }
  accessTimestamp(value.created_at, `${label}.created_at`);
  accessTimestamp(value.updated_at, `${label}.updated_at`);
  if (
    !Array.isArray(value.allowed_actions)
    || value.allowed_actions.length !== 1
    || !TENANT_ACCESS_TENANT_ACTIONS.has(value.allowed_actions[0])
    || (value.status === "active" && value.allowed_actions[0] !== "suspend")
    || (value.status === "suspended" && value.allowed_actions[0] !== "activate")
  ) {
    throw new Error(`${label}.allowed_actions无效。`);
  }
  return value;
}

function accessToolCatalogRecord(value, index) {
  const label = `available_tools[${index}]`;
  accessExactKeys(value, TENANT_ACCESS_TOOL_CATALOG_KEYS, label);
  if (typeof value.tool_name !== "string" || !TENANT_ACCESS_TOOL_NAMES.has(value.tool_name)) {
    throw new Error(`${label}.tool_name无效。`);
  }
  if (value.kind !== "read") throw new Error(`${label}.kind无效。`);
  return value;
}

function accessCredentialRecord(value, index) {
  const label = `credentials[${index}]`;
  accessExactKeys(value, TENANT_ACCESS_CREDENTIAL_KEYS, label);
  accessIdentifier(value.credential_id, `${label}.credential_id`);
  accessIdentifier(value.tenant_id, `${label}.tenant_id`);
  accessIdentifier(value.client_id, `${label}.client_id`);
  accessString(value.label, `${label}.label`);
  if (value.actor_role !== "service" || !Array.isArray(value.roles) || value.roles.length !== 1 || value.roles[0] !== "service") {
    throw new Error(`${label}的服务身份字段无效。`);
  }
  if (!Array.isArray(value.tool_names) || value.tool_names.length === 0 || value.tool_names.length > TENANT_ACCESS_TOOL_NAMES.size) {
    throw new Error(`${label}.tool_names格式无效。`);
  }
  const toolNames = new Set();
  value.tool_names.forEach((toolName, toolIndex) => {
    if (typeof toolName !== "string" || !TENANT_ACCESS_TOOL_NAMES.has(toolName) || toolNames.has(toolName)) {
      throw new Error(`${label}.tool_names[${toolIndex}]无效。`);
    }
    toolNames.add(toolName);
  });
  if (!TENANT_ACCESS_CREDENTIAL_STATUSES.has(value.status)) throw new Error(`${label}.status无效。`);
  if (!TENANT_ACCESS_DELIVERY_STATUSES.has(value.delivery_status)) throw new Error(`${label}.delivery_status无效。`);
  accessNullableTimestamp(value.delivery_acknowledged_at, `${label}.delivery_acknowledged_at`);
  if (
    (value.delivery_status === "pending" && value.delivery_acknowledged_at !== null)
    || (value.delivery_status === "acknowledged" && value.delivery_acknowledged_at === null)
  ) {
    throw new Error(`${label}的交付状态与确认时间不一致。`);
  }
  if (!TENANT_ACCESS_EFFECTIVE_STATUSES.has(value.effective_status)) throw new Error(`${label}.effective_status无效。`);
  const expectedActions = {
    pending_delivery: ["acknowledge_delivery", "revoke"],
    active: ["rotate", "revoke"],
    tenant_suspended: ["revoke"],
    expired: [],
    revoked: [],
  }[value.effective_status];
  if (
    !Array.isArray(value.allowed_actions)
    || value.allowed_actions.length !== expectedActions.length
    || value.allowed_actions.some((action) => !TENANT_ACCESS_CREDENTIAL_ACTIONS.has(action))
    || value.allowed_actions.some((action, actionIndex) => action !== expectedActions[actionIndex])
  ) {
    throw new Error(`${label}.allowed_actions无效。`);
  }
  if (
    (value.effective_status === "pending_delivery" && (value.status !== "active" || value.delivery_status !== "pending"))
    || (["active", "tenant_suspended"].includes(value.effective_status) && (value.status !== "active" || value.delivery_status !== "acknowledged"))
    || (value.effective_status === "expired" && value.status !== "expired")
    || (value.effective_status === "revoked" && value.status !== "revoked")
  ) {
    throw new Error(`${label}的生命周期状态不一致。`);
  }
  accessString(value.key_prefix, `${label}.key_prefix`);
  if (!/^lmcpk_[A-Za-z0-9._:-]+$/.test(value.key_prefix)) throw new Error(`${label}.key_prefix格式无效。`);
  accessString(value.secret_last_four, `${label}.secret_last_four`);
  if (value.secret_last_four.length !== 4) throw new Error(`${label}.secret_last_four格式无效。`);
  accessTimestamp(value.created_at, `${label}.created_at`);
  if (!Number.isSafeInteger(value.expires_at) || value.expires_at <= 0) throw new Error(`${label}.expires_at格式无效。`);
  accessNullableTimestamp(value.last_used_at, `${label}.last_used_at`);
  accessNullableTimestamp(value.revoked_at, `${label}.revoked_at`);
  accessNullableIdentifier(value.rotated_from_id, `${label}.rotated_from_id`);
  if (value.status === "active" && value.revoked_at !== null) throw new Error(`${label}的有效凭证不能带吊销时间。`);
  if (value.status === "revoked" && value.revoked_at === null) throw new Error(`${label}的吊销凭证缺少吊销时间。`);
  return value;
}

function accessOperationRecord(value, index) {
  const label = `operations[${index}]`;
  accessExactKeys(value, TENANT_ACCESS_OPERATION_KEYS, label);
  accessIdentifier(value.operation_id, `${label}.operation_id`);
  accessIdentifier(value.tenant_id, `${label}.tenant_id`);
  accessNullableIdentifier(value.credential_id, `${label}.credential_id`);
  accessIdentifier(value.actor_ref, `${label}.actor_ref`);
  if (!TENANT_ACCESS_OPERATION_ACTIONS.has(value.action)) throw new Error(`${label}.action无效。`);
  if (!TENANT_ACCESS_OPERATION_STATES.has(value.from_status)) throw new Error(`${label}.from_status无效。`);
  if (!TENANT_ACCESS_OPERATION_STATES.has(value.to_status)) throw new Error(`${label}.to_status无效。`);
  const transition = TENANT_ACCESS_OPERATION_TRANSITIONS[value.action];
  const validFrom = Array.isArray(transition.from)
    ? transition.from.includes(value.from_status)
    : transition.from === value.from_status;
  if (
    !validFrom
    || value.to_status !== transition.to
    || (transition.credential === "none" && value.credential_id !== null)
    || (transition.credential === "required" && value.credential_id === null)
  ) {
    throw new Error(`${label}的操作状态流转无效。`);
  }
  if (value.status !== "success") throw new Error(`${label}.status无效。`);
  accessIdentifier(value.reason_code, `${label}.reason_code`);
  accessTimestamp(value.created_at, `${label}.created_at`);
  return value;
}

export function validateTenantAccessState(value) {
  accessExactKeys(value, TENANT_ACCESS_STATE_KEYS, "tenant_access_state");
  if (!Array.isArray(value.available_tools) || value.available_tools.length === 0 || value.available_tools.length > TENANT_ACCESS_TOOL_NAMES.size) {
    throw new Error("可配置工具目录格式无效。");
  }
  if (!Array.isArray(value.tenants) || value.tenants.length > 1_024) throw new Error("租户列表格式无效。");
  if (!Array.isArray(value.credentials) || value.credentials.length > 4_096) throw new Error("凭证列表格式无效。");
  if (!Array.isArray(value.operations) || value.operations.length > 256) throw new Error("操作状态流转格式无效。");
  value.available_tools.forEach(accessToolCatalogRecord);
  const availableToolNames = new Set(value.available_tools.map((item) => item.tool_name));
  if (availableToolNames.size !== value.available_tools.length) throw new Error("可配置工具目录包含重复项。");
  value.tenants.forEach(accessTenantRecord);
  value.credentials.forEach(accessCredentialRecord);
  value.operations.forEach(accessOperationRecord);
  const tenantIds = new Set(value.tenants.map((item) => item.tenant_id));
  if (value.credentials.some((item) => !tenantIds.has(item.tenant_id)) || value.operations.some((item) => !tenantIds.has(item.tenant_id))) {
    throw new Error("租户与凭证/操作关联不一致。");
  }
  if (value.credentials.some((item) => item.tool_names.some((toolName) => !availableToolNames.has(toolName)))) {
    throw new Error("凭证 tool_names 不在服务端可配置工具目录中。");
  }
  const tenantStatuses = new Map(value.tenants.map((item) => [item.tenant_id, item.status]));
  if (value.credentials.some((item) => (
    (item.effective_status === "active" && tenantStatuses.get(item.tenant_id) !== "active")
    || (item.effective_status === "tenant_suspended" && tenantStatuses.get(item.tenant_id) !== "suspended")
  ))) {
    throw new Error("租户与凭证有效状态不一致。");
  }
  return value;
}

function validateAccessSecretDelivery(value) {
  if (!isRecord(value)) throw new Error("凭证交付状态格式无效。");
  const keys = ["status", "credential_id"];
  accessExactKeys(value, keys, "secret_delivery");
  if (value.status !== "one_time" && value.status !== "withheld") throw new Error("凭证交付状态无效。");
  accessIdentifier(value.credential_id, "secret_delivery.credential_id");
  return value;
}

function validateTenantAccessEnvelope(value) {
  if (!isRecord(value)) throw new Error("tenant_access_envelope必须是对象。");
  const actualKeys = Object.keys(value);
  if (
    actualKeys.some((key) => !TENANT_ACCESS_ENVELOPE_KEYS.includes(key))
    || ["schema_version", "status", "data", "reason_codes"].some((key) => !Object.hasOwn(value, key))
  ) {
    throw new Error("tenant_access_envelope字段不完整或包含未知字段。");
  }
  if (value.schema_version !== TENANT_ACCESS_SCHEMA_VERSION) throw new Error("Tenant Access 返回版本无效。");
  if (!TENANT_ACCESS_RESPONSE_STATUSES.has(value.status)) throw new Error("Tenant Access 返回状态无效。");
  if (value.reason_codes !== undefined) {
    if (!Array.isArray(value.reason_codes) || value.reason_codes.some((reason) => typeof reason !== "string" || !TENANT_ACCESS_IDENTIFIER_PATTERN.test(reason))) {
      throw new Error("Tenant Access 原因码格式无效。");
    }
  }
  if (value.replayed !== undefined && typeof value.replayed !== "boolean") throw new Error("Tenant Access 重放标记无效。");
  if (Object.hasOwn(value, "secret_delivery")) validateAccessSecretDelivery(value.secret_delivery);
  if (value.data !== null && !isRecord(value.data)) throw new Error("Tenant Access 返回数据无效。");
  return value;
}

function removeAccessSecret(value) {
...
export class TenantAccessError extends Error {
  constructor(message, { status = "unavailable", reasonCodes = [], data = null } = {}) {
    ...
    this.status = status;
    this.reasonCodes = Array.isArray(reasonCodes) ? reasonCodes.filter((reason) => typeof reason === "string") : [];
    this.data = removeAccessSecret(data);
  }
}

function accessRequestIdempotencyKey(action) {
  const suffix = typeof globalThis.crypto?.randomUUID === "function"
    ? globalThis.crypto.randomUUID()
    : `${Date.now()}-${Math.random().toString(36).slice(2)}`;
  return `admin-access-${action}-${suffix}`;
}

function accessResolvedIdempotencyKey(value, action) {
  if (value === undefined) return accessRequestIdempotencyKey(action);
  if (typeof value !== "string" || !/^[A-Za-z0-9][A-Za-z0-9._:-]{15,199}$/.test(value)) {
    throw new TenantAccessError("幂等键格式无效。", { status: "needs_input" });
  }
  return value;
}

function accessSecretDeliveryStatus(value) {
  return isRecord(value) ? value.status : undefined;
}

function validateOneTimeCredentialResult(envelope) {
  const data = envelope.data;
  if (!isRecord(data) || !isRecord(data.credential) || typeof data.api_key !== "string" || !TENANT_ACCESS_API_KEY_PATTERN.test(data.api_key)) {
    throw new TenantAccessError("凭证响应未通过格式校验。", { status: "manual_review" });
  }
  accessCredentialRecord(data.credential, 0);
  if (!isRecord(data.operation)) throw new TenantAccessError("凭证操作回读缺失。", { status: "manual_review" });
  accessOperationRecord(data.operation, 0);
  if (accessSecretDeliveryStatus(envelope.secret_delivery) !== "one_time") {
    throw new TenantAccessError("凭证不是一次性签发响应。", { status: "manual_review", data });
  }
  return data;
}

function validateAccessWriteResult(envelope, action) {
  if (action === "issue" || action === "rotate") return validateOneTimeCredentialResult(envelope);
  if (!isRecord(envelope.data)) throw new TenantAccessError("Tenant Access 写入响应未通过格式校验。", { status: "manual_review" });
  if (!isRecord(envelope.data.operation)) throw new TenantAccessError("操作精确回读缺失。", { status: "manual_review" });
  accessOperationRecord(envelope.data.operation, 0);
  if (action === "createTenant" || action === "setTenantStatus") {
    if (!isRecord(envelope.data.tenant)) throw new TenantAccessError("租户写入响应未通过格式校验。", { status: "manual_review" });
    accessTenantRecord(envelope.data.tenant, 0);
  }
  if ((action === "revoke" || action === "acknowledgeDelivery") && !isRecord(envelope.data.credential)) {
    throw new TenantAccessError("凭证吊销响应未通过格式校验。", { status: "manual_review" });
  }
  if (action === "revoke" || action === "acknowledgeDelivery") accessCredentialRecord(envelope.data.credential, 0);
  return envelope.data;
}

export function createTenantAccessClient({
  fetchImpl = globalThis.fetch,
  basePath = TENANT_ACCESS_API_ROOT,
  allowWrites = true,
} = {}) {
  if (typeof fetchImpl !== "function") throw new TypeError("需要提供 Access 请求实现。");
  ...
  async function request(path, { method = "GET", body, idempotencyKey, action } = {}) {
    if (bearerToken === "") throw new TenantAccessError("请先绑定控制面身份。", { status: "needs_input" });
    if (method === "POST" && allowWrites !== true) {
      throw new TenantAccessError("正式路径不开放 Tenant Access 写操作。", { status: "blocked" });
    }
    ...
    if (method === "POST") headers.set("idempotency-key", accessResolvedIdempotencyKey(idempotencyKey, action ?? "write"));
    let response;
    try {
      const endpoint = path.startsWith("/") ? path : `${root}/${path.replace(/^\//, "")}`;
      ...
      throw new TenantAccessError("Tenant Access 请求不可用。", { status: "unavailable" });
    }
    let payload;
    try {
      payload = await response.json();
    } catch {
      throw new TenantAccessError("Tenant Access 返回格式无效。", { status: "unavailable" });
    }
    let envelope;
    try {
      envelope = validateTenantAccessEnvelope(payload);
    } catch {
      throw new TenantAccessError("Tenant Access 返回格式无效。", { status: "unavailable" });
    }
    const status = !response.ok && envelope.status === "success"
      ? response.status >= 500 ? "unavailable" : "blocked"
      : envelope.status;
    if (status !== "success") {
      throw new TenantAccessError("Tenant Access 操作未完成。", {
        status,
    ...
    try {
      if (action === "state") return validateTenantAccessState(envelope.data);
      return validateAccessWriteResult(envelope, action);
    } catch (error) {
      if (error instanceof TenantAccessError) throw error;
      throw new TenantAccessError("Tenant Access 成功响应未通过格式校验。", { status: "manual_review" });
    }
  }
  ...
  return Object.freeze({
    ...
    async getState() {
      return request(root === TENANT_ACCESS_API_ROOT ? TENANT_ACCESS_STATE_PATH : "state", { action: "state" });
    },
    async createTenant(payload, idempotencyKey) {
      return request("tenants", { method: "POST", body: payload, idempotencyKey, action: "createTenant" });
    },
    async setTenantStatus(tenantId, payload, idempotencyKey) {
      accessIdentifier(tenantId, "tenant_id");
      return request(`tenants/${encodeURIComponent(tenantId)}/status`, { method: "POST", body: payload, idempotencyKey, action: "setTenantStatus" });
    },
    async issueCredential(payload, idempotencyKey) {
      return request("credentials", { method: "POST", body: payload, idempotencyKey, action: "issue" });
    },
    async rotateCredential(credentialId, payload, idempotencyKey) {
      accessIdentifier(credentialId, "credential_id");
      return request(`credentials/${encodeURIComponent(credentialId)}/rotate`, { method: "POST", body: payload, idempotencyKey, action: "rotate" });
    },
    async revokeCredential(credentialId, payload, idempotencyKey) {
      accessIdentifier(credentialId, "credential_id");
      return request(`credentials/${encodeURIComponent(credentialId)}/revoke`, { method: "POST", body: payload, idempotencyKey, action: "revoke" });
    },
    async acknowledgeCredentialDelivery(credentialId, payload, idempotencyKey) {
      accessIdentifier(credentialId, "credential_id");
      return request(`credentials/${encodeURIComponent(credentialId)}/acknowledge-delivery`, {
        method: "POST",
        body: payload,
        idempotencyKey,
...
const VIEW_META = {
  ...
  "tenant-access": {
    ...
    description: "管理租户状态与机器客户端凭证；新 Key 只显示一次，确认安全保存并精确读回操作后才可调用。",
  },
  tools: {
    title: "工具权限",
    eyebrow: "角色权限清单",
...
const state = isBrowser
  ? {
      ...
      controlNotice: null,
      accessState: null,
      accessStateLoading: false,
      accessStatus: "needs_input",
      accessError: null,
      accessActor: null,
      accessNotice: null,
    }
...
const tenantAccessClient = isBrowser
  ? createTenantAccessClient({ allowWrites: state.mode === "fixture" })
  : null;
...
const accessIdentityDialog = isBrowser ? document.querySelector("#access-identity-dialog") : null;
const credentialToolDialog = isBrowser ? document.querySelector("#credential-tool-dialog") : null;
const oneTimeCredentialDialog = isBrowser ? document.querySelector("#one-time-credential-dialog") : null;
let dialogTrigger = null;
let identityDialogTrigger = null;
let accessIdentityDialogTrigger = null;
let credentialToolDialogTrigger = null;
let credentialToolCredentialId = null;
let oneTimeCredentialId = null;
let oneTimeCredentialReadbackVerified = false;
...
function renderModuleCenter(data) {
  ...
  return `${pageHeader("modules", `<button class="button button-secondary" type="button" data-view="tenant-access">配置 Key 功能</button><button class="button button-secondary" type="button" data-control-action="refresh">重新读取</button>`)}
    ...
    <div class="callout callout-info" role="note"><div class="callout-head"><h2>模块开关与客户端功能分开管理</h2>${controlStatusMarkup("complete", "边界明确")}</div><p>这里控制模块是否经过发布并进入运行时；某个租户 API Key 可调用哪些具体工具，请进入“租户与 API Key”按工具名精确配置。授权不会绕过模块发布或依赖就绪门禁。</p></div>
    ${controlNoticeMarkup()}
...
const ACCESS_STATUS_META = {
  active: { status: "success", label: "启用" },
  pending_delivery: { status: "needs_input", label: "待确认交付" },
  tenant_suspended: { status: "manual_review", label: "租户暂停" },
  suspended: { status: "manual_review", label: "已暂停" },
  expired: { status: "blocked", label: "已过期" },
  revoked: { status: "blocked", label: "已吊销" },
};

function accessStatusMarkup(status, label) {
  const meta = ACCESS_STATUS_META[status] ?? { status: "empty", label: "未返回" };
  return statusMarkup(meta.status, label ?? meta.label);
}

function accessControlStatusMarkup(status) {
  if (status === "complete") return statusMarkup("success", "已读回");
  if (status === "loading") return statusMarkup("loading", "读取中");
  return statusMarkup(status);
}

function accessToolMarkup(toolNames) {
  if (!Array.isArray(toolNames) || toolNames.length === 0) return statusMarkup("empty", "未返回功能");
  return `<div class="access-scope-list">${toolNames.map((toolName) => `<span class="access-scope-chip" title="${escapeHtml(toolName)}">${escapeHtml(TENANT_ACCESS_TOOL_LABELS[toolName] ?? toolName)}</span>`).join("")}</div>`;
}

function accessToolOptionsMarkup(availableTools, selectedToolNames, disabled = "") {
  const selected = new Set(selectedToolNames);
  return availableTools.map((tool) => {
    const toolName = tool.tool_name;
    const label = TENANT_ACCESS_TOOL_LABELS[toolName] ?? toolName;
    return `<label class="scope-option"><input type="checkbox" name="tool_names" value="${escapeHtml(toolName)}"${selected.has(toolName) ? " checked" : ""}${disabled} /><span><strong>${escapeHtml(label)}</strong><small>${escapeHtml(toolName)}</small></span></label>`;
  }).join("");
}

function accessDateMarkup(value) {
  if (!Number.isSafeInteger(value) || value <= 0) return "未返回";
  const date = new Date(value * 1_000);
  if (!Number.isFinite(date.getTime())) return "未返回";
  return escapeHtml(date.toLocaleString("zh-CN", { hour12: false }));
}

function accessWriteAttributes(action) {
  if (state.mode === "fixture") return ` data-access-write="${escapeHtml(action)}"`;
  return ` disabled aria-disabled="true" title="正式路径写操作固定关闭" data-access-write="${escapeHtml(action)}"`;
}

function accessCredentialActionAttributes(credential, allowedAction, writeAction) {
  if (credential.allowed_actions.includes(allowedAction)) return accessWriteAttributes(writeAction);
  const reason = credential.effective_status === "pending_delivery"
    ? "待交付凭证只能确认保存或吊销"
    : credential.effective_status === "tenant_suspended"
      ? "租户暂停期间不能轮换凭证"
      : credential.status === "expired"
        ? "已过期凭证不能执行此操作"
        : "当前服务端状态不允许此操作";
  return ` disabled aria-disabled="true" title="${reason}" data-access-write="${escapeHtml(writeAction)}"`;
}

function accessNoticeMarkup() {
  if (!state.accessNotice) return "";
  const notice = state.accessNotice;
  const mappedStatus = ACCESS_STATUS_META[notice.status]?.status ?? notice.status;
  return `<div class="callout callout-${escapeHtml(notice.kind)} access-notice" role="status"><div class="callout-head"><h2>${escapeHtml(notice.title)}</h2>${statusMarkup(mappedStatus, notice.label)}</div><p>${escapeHtml(notice.detail)}</p></div>`;
}

function renderAccessBoundary() {
  if (state.mode === "fixture") {
    return `<div class="callout callout-warning access-boundary" role="note"><div class="callout-head"><h2>本地演示边界</h2>${statusMarkup("manual_review", "仅限本地")}</div><p>状态仍以 Tenant Access 服务端读回为准；签发、轮换、吊销和状态切换只在本地演示路径尝试，不使用内置演示租户。</p></div>`;
  }
  return `<div class="callout callout-warning access-boundary" role="alert"><div class="callout-head"><h2>正式路径写操作已关闭</h2>${statusMarkup("blocked", "不可签发")}</div><p>当前页面只允许同源读取已脱敏状态。创建租户、状态切换、签发、轮换和吊销在非 fixture 路径固定阻断；本页面不代表生产资格。</p></div>`;
}

function renderAccessIdentityPanel() {
  const fixture = state.mode === "fixture" && isFixtureIdentityVisible(window.location.search);
  const actions = fixture
    ? `<div class="identity-actions"><button class="button button-primary" type="button" data-access-action="fixture-identity" data-identity="local_operator">本地演示申请人</button><button class="button button-secondary" type="button" data-access-action="fixture-identity" data-identity="local_approver">本地演示审批人</button></div><p class="field-help">身份只在当前页面内存中使用；按钮不显示凭证值。</p>`
    : `<button class="button button-primary" type="button" data-access-action="open-identity">输入控制面身份</button><p class="field-help">Bearer 身份只保留在本次页面会话的请求客户端中，不写入存储、地址栏、页面文本或日志。</p>`;
  const bound = state.accessActor ? `<p class="bound-identity">当前身份：<strong>${escapeHtml(state.accessActor.label)}</strong>。状态以服务端读回为准。</p>` : "";
  const error = state.accessError ? `<p class="field-help">${escapeHtml(state.accessError)}</p>` : "";
  return `<section class="panel identity-panel access-identity-panel" aria-labelledby="access-identity-title"><div class="card-head"><div><h2 id="access-identity-title">绑定 Tenant Access 身份</h2><p>只有服务端认证后的管理身份才能读取租户和凭证元数据；客户端不能自报租户。</p></div>${accessControlStatusMarkup(state.accessStatus)}</div>${bound}${error}${actions}</section>`;
}

function renderAccessCreateTenantForm() {
  const disabled = state.mode === "fixture" ? "" : " disabled aria-disabled=\"true\" title=\"正式路径写操作固定关闭\"";
  return `<section class="panel access-form-panel" aria-labelledby="access-create-tenant-title"><div class="card-head"><div><h2 id="access-create-tenant-title">创建租户</h2><p>只建立接入身份目录，不创建客户、订单或业务主数据。</p></div>${state.mode === "fixture" ? statusMarkup("manual_review", "本地受控") : statusMarkup("blocked", "正式关闭")}</div><form id="access-create-tenant-form" class="access-form" novalidate><div class="field"><label for="access-tenant-id">租户标识</label><input id="access-tenant-id" name="tenant_id" type="text" inputmode="text" autocomplete="off" maxlength="128" required${disabled} /></div><div class="field"><label for="access-tenant-name">租户名称</label><input id="access-tenant-name" name="display_name" type="text" autocomplete="off" maxlength="120" required${disabled} /></div><p class="field-help">提交后输入立即清空；重复请求使用新的幂等键。</p><div class="button-row"><button class="button button-primary" type="submit" data-access-action="create-tenant"${disabled}>创建租户</button></div></form></section>`;
}

function renderAccessIssueForm(tenants, availableTools) {
  const activeTenants = tenants.filter((tenant) => tenant.status === "active");
  const disabled = state.mode === "fixture" && activeTenants.length > 0 ? "" : " disabled aria-disabled=\"true\" title=\"没有可签发的本地启用租户\"";
  const options = activeTenants.length
    ? activeTenants.map((tenant) => `<option value="${escapeHtml(tenant.tenant_id)}">${escapeHtml(tenant.display_name)}</option>`).join("")
    : `<option value="">暂无启用租户</option>`;
  const availableNames = availableTools.map((tool) => tool.tool_name);
  const preferredDefaults = ["cargo.calculate", "container.plan_summary"].filter((toolName) => availableNames.includes(toolName));
  const selectedDefaults = preferredDefaults.length > 0 ? preferredDefaults : availableNames.slice(0, 1);
  const toolOptions = accessToolOptionsMarkup(availableTools, selectedDefaults, disabled);
  return `<section class="panel access-form-panel" aria-labelledby="access-issue-title"><div class="card-head"><div><h2 id="access-issue-title">签发机器凭证</h2><p>凭证固定映射为服务身份，并按 MCP 工具名精确授权；完整 key 不会进入租户列表。</p></div>${state.mode === "fixture" ? statusMarkup("manual_review", "一次性显示") : statusMarkup("blocked", "正式关闭")}</div><form id="access-issue-form" class="access-form" novalidate><div class="field"><label for="access-issue-tenant">所属租户</label><select id="access-issue-tenant" name="tenant_id" required${disabled}>${options}</select></div><div class="field"><label for="access-client-id">客户端标识</label><input id="access-client-id" name="client_id" type="text" autocomplete="off" maxlength="128" required${disabled} /></div><div class="field"><label for="access-client-label">客户端名称</label><input id="access-client-label" name="label" type="text" autocomplete="off" maxlength="120" required${disabled} /></div><fieldset class="scope-fieldset"${disabled}><legend>可调用插件功能</legend><div class="scope-options">${toolOptions}</div></fieldset><div class="field"><label for="access-expires">有效期</label><select id="access-expires" name="expires_in_seconds" required${disabled}><option value="86400">1 天</option><option value="604800">7 天</option><option value="2592000">30 天</option></select></div><p class="field-help">授权只决定该 Key 能看到和调用哪些工具，不代表对应模块已获得生产资格；运行时仍会校验模块状态、租户状态和有效期。</p><div class="button-row"><button class="button button-primary" type="submit" data-access-action="issue-credential"${disabled}>签发并显示一次</button></div></form></section>`;
}

function renderAccessTenantCards(tenants) {
  if (tenants.length === 0) return emptyState("暂无租户", "服务端没有返回租户记录；不根据输入或演示数据补造租户。");
  return `<section class="panel access-list-panel" aria-labelledby="access-tenant-list-title"><div class="card-head"><div><h2 id="access-tenant-list-title">租户目录</h2><p>按钮只使用服务端返回的 allowed_actions，不由页面自行推断下一状态。</p></div>${statusMarkup("ready", `${tenants.length} 个租户`)}</div><div class="access-tenant-grid">${tenants.map((tenant) => {
    const canActivate = tenant.allowed_actions.includes("activate");
    const canSuspend = tenant.allowed_actions.includes("suspend");
    return `<article class="access-tenant-card"><div class="access-card-head"><div><h3>${display(tenant.display_name, "未命名租户")}</h3><p>标识已登记（具体内容隐藏）</p></div>${accessStatusMarkup(tenant.status)}</div><dl class="access-meta-list"><div><dt>状态更新时间</dt><dd>${display(tenant.updated_at, "未返回")}</dd></div><div><dt>凭证数量</dt><dd>${state.accessState.credentials.filter((credential) => credential.tenant_id === tenant.tenant_id).length} 个</dd></div></dl><div class="button-row access-card-actions"><button class="button button-secondary" type="button" data-access-action="set-tenant-status" data-tenant-id="${escapeHtml(tenant.tenant_id)}" data-tenant-status="active"${canActivate ? accessWriteAttributes("set-tenant-status-active") : " disabled aria-disabled=\"true\" title=\"服务端未允许恢复启用\""}>恢复启用</button><button class="button button-secondary" type="button" data-access-action="set-tenant-status" data-tenant-id="${escapeHtml(tenant.tenant_id)}" data-tenant-status="suspended"${canSuspend ? accessWriteAttributes("set-tenant-status-suspended") : " disabled aria-disabled=\"true\" title=\"服务端未允许暂停\""}>暂停租户</button></div></article>`;
  }).join("")}</div></section>`;
}

function renderAccessCredentialTable(credentials) {
  if (credentials.length === 0) return emptyState("暂无机器凭证", "没有签发成功的凭证元数据；完整 key 不会从状态接口恢复。");
  return `<section class="panel access-list-panel" aria-labelledby="access-credential-list-title"><div class="card-head"><div><h2 id="access-credential-list-title">机器凭证</h2><p>待交付凭证在确认安全保存前不能调用 MCP；调整功能会吊销旧 Key，并生成待交付的新 Key。</p></div>${statusMarkup("ready", `${credentials.length} 条记录`)}</div><div class="table-scroll access-table-scroll" role="region" aria-label="机器凭证脱敏列表，可横向滚动" tabindex="0"><table class="data-table access-credential-table"><thead><tr><th scope="col">客户端</th><th scope="col">凭证标识</th><th scope="col">密钥脱敏信息</th><th scope="col">插件功能</th><th scope="col">运行状态</th><th scope="col">交付状态</th><th scope="col">到期时间</th><th scope="col">操作</th></tr></thead><tbody>${credentials.map((credential) => `<tr><td><span class="primary-cell">${display(credential.label, "未命名客户端")}</span><span class="sub-cell">客户端标识已隐藏</span></td><td><span class="sub-cell">已登记（具体标识隐藏）</span></td><td><span class="access-key-prefix">${escapeHtml(credential.key_prefix)}</span><span class="sub-cell">末四位：${escapeHtml(credential.secret_last_four)}</span></td><td>${accessToolMarkup(credential.tool_names)}</td><td>${accessStatusMarkup(credential.effective_status)}</td><td>${credential.delivery_status === "acknowledged" ? statusMarkup("success", "已确认保存") : statusMarkup("needs_input", "待确认交付")}</td><td>${accessDateMarkup(credential.expires_at)}</td><td><div class="button-row access-credential-actions"><button class="button button-secondary" type="button" data-access-action="rotate-credential" data-credential-id="${escapeHtml(credential.credential_id)}"${accessCredentialActionAttributes(credential, "rotate", "rotate-credential")}>调整功能</button><button class="button button-secondary" type="button" data-access-action="revoke-credential" data-credential-id="${escapeHtml(credential.credential_id)}"${accessCredentialActionAttributes(credential, "revoke", "revoke-credential")}>吊销</button></div></td></tr>`).join("")}</tbody></table></div></section>`;
}

const ACCESS_OPERATION_LABELS = Object.freeze({
  "tenant.create": "创建租户",
  "tenant.activate": "恢复租户",
  "tenant.suspend": "暂停租户",
  "credential.issue": "签发凭证",
  "credential.delivery_acknowledge": "确认安全交付",
  "credential.rotate": "轮换凭证",
  "credential.revoke": "吊销凭证",
});

const ACCESS_OPERATION_STATE_LABELS = Object.freeze({
  absent: "未建立",
  active: "已启用",
  suspended: "已暂停",
  pending_delivery: "待确认交付",
  tenant_suspended: "租户暂停",
  expired: "已过期",
  revoked: "已吊销",
});

function renderAccessOperationFlow(operations) {
  const rows = operations.slice(0, 12);
  return `<section class="panel access-operation-panel" aria-labelledby="access-operation-title"><div class="card-head"><div><h2 id="access-operation-title">操作状态流转</h2><p>写操作只在请求校验、事务落库和 operation ID 精确回读都完成后显示成功。</p></div>${statusMarkup(rows.length ? "ready" : "empty", rows.length ? `${rows.length} 条最近操作` : "暂无操作")}</div><ol class="access-flow-rail" aria-label="操作校验阶段"><li><strong>1. 输入与权限校验</strong><span>失败则停在需要补充或已阻断</span></li><li><strong>2. 服务端事务落库</strong><span>对象状态、幂等和审计同步收敛</span></li><li><strong>3. operation ID 精确回读</strong><span>读回缺失或不一致转人工复核</span></li><li><strong>4. 一次性 Key 安全交付</strong><span>未确认安全保存前不可调用 MCP</span></li></ol>${rows.length ? `<div class="table-scroll access-table-scroll" role="region" aria-label="最近操作状态流转" tabindex="0"><table class="data-table access-operation-table"><thead><tr><th scope="col">操作</th><th scope="col">状态流转</th><th scope="col">结果</th><th scope="col">时间</th></tr></thead><tbody>${rows.map((operation) => `<tr><td><span class="primary-cell">${escapeHtml(ACCESS_OPERATION_LABELS[operation.action] ?? "受控操作")}</span><span class="sub-cell">operation 已记录（标识隐藏）</span></td><td>${escapeHtml(ACCESS_OPERATION_STATE_LABELS[operation.from_status] ?? "未知")} → ${escapeHtml(ACCESS_OPERATION_STATE_LABELS[operation.to_status] ?? "未知")}</td><td>${statusMarkup(operation.status, "服务端已终结")}</td><td>${display(operation.created_at, "未返回")}</td></tr>`).join("")}</tbody></table></div>` : emptyState("暂无操作记录", "没有 operation 回读时不自行构造状态轨迹。")}</section>`;
}

function renderTenantAccess() {
  const data = state.accessState;
  const availableTools = Array.isArray(data?.available_tools) ? data.available_tools : [];
  const tenants = Array.isArray(data?.tenants) ? data.tenants : [];
  const credentials = Array.isArray(data?.credentials) ? data.credentials : [];
  const operations = Array.isArray(data?.operations) ? data.operations : [];
  const activeCredentials = credentials.filter((credential) => credential.effective_status === "active").length;
  const pendingDelivery = credentials.filter((credential) => credential.effective_status === "pending_delivery").length;
  const body = !state.accessActor
    ? `<section class="panel access-empty-panel"><div class="empty-state"><span class="status-pill status-needs_input"><span class="status-icon" aria-hidden="true">!</span>需要身份</span><h2>先绑定控制面身份</h2><p>读取租户和凭证状态前必须完成服务端 Bearer 认证；没有身份时不发起 Access 请求。</p></div></section>`
    : state.accessStateLoading
      ? `<section class="panel loading-panel access-loading-panel" aria-live="polite">${statusMarkup("loading")}<h2>正在读取 Tenant Access</h2><p>只读取服务端状态；未返回前不显示假租户、假凭证或假状态。</p></section>`
      : state.accessError && data === null
        ? `<section class="panel access-empty-panel"><div class="empty-state">${statusMarkup(state.accessStatus, "状态不可用")}<h2>Tenant Access 状态不可用</h2><p>${escapeHtml(state.accessError)}</p><p>页面不会回退到 fixture-data 或内置默认记录。</p></div></section>`
        : `<div class="metric-grid access-metric-grid" aria-label="Tenant Access 状态"><article class="metric-card"><div class="metric-top"><span class="metric-label">租户总数</span><span class="metric-icon" data-icon="access-tenant" aria-hidden="true"></span></div><div class="metric-value">${tenants.length}</div><div class="metric-detail">服务端状态读回</div></article><article class="metric-card"><div class="metric-top"><span class="metric-label">可配置功能</span><span class="metric-icon" data-icon="access-key" aria-hidden="true"></span></div><div class="metric-value">${availableTools.length}</div><div class="metric-detail">服务端工具白名单</div></article><article class="metric-card"><div class="metric-top"><span class="metric-label">可调用凭证</span><span class="metric-icon" data-icon="access-key" aria-hidden="true"></span></div><div class="metric-value">${activeCredentials}</div><div class="metric-detail">已确认安全交付</div></article><article class="metric-card"><div class="metric-top"><span class="metric-label">待交付凭证</span><span class="metric-icon" data-icon="access-key" aria-hidden="true"></span></div><div class="metric-value">${pendingDelivery}</div><div class="metric-detail">确认前不可调用 MCP</div></article></div>${renderAccessOperationFlow(operations)}<div class="access-form-grid">${renderAccessCreateTenantForm()}${renderAccessIssueForm(tenants, availableTools)}</div>${renderAccessTenantCards(tenants)}${renderAccessCredentialTable(credentials)}`;
  return `${pageHeader("tenant-access", `<button class="button button-secondary" type="button" data-access-action="refresh">重新读取</button>`)}${renderAccessBoundary()}${accessNoticeMarkup()}${renderAccessIdentityPanel()}${body}`;
}
...
function renderView() {
  if (state.view === "tenant-access") return renderTenantAccess();
...
function accessErrorNotice(error, actionLabel) {
  const status = error instanceof TenantAccessError && TENANT_ACCESS_RESPONSE_STATUSES.has(error.status)
    ? error.status
    : "unavailable";
  const detail = status === "needs_input"
    ? "请先绑定控制面身份；页面没有发起无身份请求。"
    : status === "blocked"
      ? `${actionLabel}已被服务端或当前环境阻断；页面没有把操作显示为成功。`
      : status === "manual_review"
        ? `${actionLabel}需要人工复核；页面保留现状，不显示未经核验的凭证。`
        : `${actionLabel}未完成；服务端状态不可确认，页面保持失败闭合。`;
  return { status, detail };
}

function setAccessState(accessState) {
  state.accessState = validateTenantAccessState(accessState);
}

async function loadAccessState({ announce = false, preserveNotice = false } = {}) {
  if (!tenantAccessClient || !state.accessActor) {
    state.accessStatus = "needs_input";
    state.accessStateLoading = false;
    render(announce);
    return false;
  }
  state.accessStateLoading = true;
  state.accessStatus = "loading";
  state.accessError = null;
  render();
  try {
    setAccessState(await tenantAccessClient.getState());
    state.accessStatus = "complete";
    if (!preserveNotice) state.accessNotice = null;
    return true;
  } catch (error) {
    const failure = accessErrorNotice(error, "读取 Tenant Access 状态");
    state.accessStatus = failure.status;
    state.accessError = failure.detail;
    return false;
  } finally {
    state.accessStateLoading = false;
    render(announce);
...
function openAccessIdentityDialog() {
  if (!accessIdentityDialog) return;
  accessIdentityDialogTrigger = document.activeElement instanceof HTMLButtonElement ? document.activeElement : null;
  const input = document.querySelector("#access-identity-token");
  const error = document.querySelector("#access-identity-form-error");
  if (input instanceof HTMLInputElement) input.value = "";
  if (error instanceof HTMLElement) error.hidden = true;
  if (typeof accessIdentityDialog.showModal === "function") accessIdentityDialog.showModal();
  else accessIdentityDialog.setAttribute("open", "");
  queueMicrotask(() => input?.focus());
}

function closeAccessIdentityDialog() {
  if (!accessIdentityDialog) return;
  const input = document.querySelector("#access-identity-token");
  const error = document.querySelector("#access-identity-form-error");
  if (input instanceof HTMLInputElement) input.value = "";
  if (error instanceof HTMLElement) error.hidden = true;
  if (typeof accessIdentityDialog.close === "function") accessIdentityDialog.close();
  else accessIdentityDialog.removeAttribute("open");
  const trigger = accessIdentityDialogTrigger;
  accessIdentityDialogTrigger = null;
  queueMicrotask(() => trigger?.focus());
}

function bindAccessIdentity(identity) {
  if (!tenantAccessClient || !identity) return;
  tenantAccessClient.setToken(identity.token);
  state.accessActor = {
  ...
  closeAccessIdentityDialog();
  void loadAccessState({ announce: true });
}

function submitAccessIdentity() {
  const input = document.querySelector("#access-identity-token");
  ...
  if (error instanceof HTMLElement) error.hidden = true;
  bindAccessIdentity({ label: "已绑定控制面身份", role: "admin", token });
}

function closeCredentialToolDialog() {
  credentialToolCredentialId = null;
  if (!credentialToolDialog) return;
  const options = document.querySelector("#credential-tool-options");
  const error = document.querySelector("#credential-tool-form-error");
  if (options instanceof HTMLElement) options.replaceChildren();
  if (error instanceof HTMLElement) error.hidden = true;
  if (typeof credentialToolDialog.close === "function") credentialToolDialog.close();
  else credentialToolDialog.removeAttribute("open");
  const trigger = credentialToolDialogTrigger;
  credentialToolDialogTrigger = null;
  queueMicrotask(() => trigger?.focus());
}

function openCredentialToolDialog(credentialId) {
  if (!credentialToolDialog || !state.accessState) return;
  const credential = state.accessState.credentials.find((item) => item.credential_id === credentialId);
  if (!credential || !credential.allowed_actions.includes("rotate")) return;
  const options = document.querySelector("#credential-tool-options");
  const error = document.querySelector("#credential-tool-form-error");
  if (!(options instanceof HTMLElement)) return;
  credentialToolDialogTrigger = document.activeElement instanceof HTMLButtonElement ? document.activeElement : null;
  credentialToolCredentialId = credentialId;
  options.innerHTML = accessToolOptionsMarkup(
    state.accessState.available_tools,
    credential.tool_names,
  );
  if (error instanceof HTMLElement) error.hidden = true;
  if (typeof credentialToolDialog.showModal === "function") credentialToolDialog.showModal();
  else credentialToolDialog.setAttribute("open", "");
  queueMicrotask(() => options.querySelector("input:checked")?.focus());
}

async function submitCredentialToolForm(form) {
  if (!tenantAccessClient || state.mode !== "fixture" || credentialToolCredentialId === null) return;
  const toolNames = accessFormTexts(form, "tool_names");
  const expiresInSeconds = Number(accessFormText(form, "expires_in_seconds"));
  const error = document.querySelector("#credential-tool-form-error");
  if (toolNames.length === 0 || !Number.isSafeInteger(expiresInSeconds)) {
    if (error instanceof HTMLElement) error.hidden = false;
    return;
  }
  if (error instanceof HTMLElement) error.hidden = true;
  const credentialId = credentialToolCredentialId;
  closeCredentialToolDialog();
  await runAccessOperation("调整功能并轮换", () => tenantAccessClient.rotateCredential(credentialId, {
    schema_version: TENANT_ACCESS_SCHEMA_VERSION,
    tool_names: toolNames,
    expires_in_seconds: expiresInSeconds,
    reason_code: "operator_function_profile_changed",
  }, undefined), { showSecret: true, expectedToolNames: toolNames });
}

function clearOneTimeCredentialDialog({ acknowledged = false, announce = true } = {}) {
  const pendingCredentialId = oneTimeCredentialId;
  oneTimeCredentialId = null;
  oneTimeCredentialReadbackVerified = false;
  if (!oneTimeCredentialDialog) return;
  const body = document.querySelector("#one-time-credential-body");
  const input = document.querySelector("#one-time-api-key");
  if (input instanceof HTMLInputElement) input.value = "";
  if (body instanceof HTMLElement) body.replaceChildren();
  if (typeof oneTimeCredentialDialog.close === "function") oneTimeCredentialDialog.close();
  else oneTimeCredentialDialog.removeAttribute("open");
  if (!acknowledged && announce && pendingCredentialId !== null) {
    state.accessNotice = {
      kind: "warning",
      status: "manual_review",
      title: "凭证仍待交付确认",
      detail: "完整 Key 已从页面清除且无法恢复；该凭证保持待交付状态，不能调用 MCP。请吊销后重新签发。",
      label: "需吊销重签",
    };
    render(true);
  }
...
function openOneTimeCredentialDialog(apiKey, credentialId, { readbackVerified = false } = {}) {
  if (
    !oneTimeCredentialDialog
    || typeof apiKey !== "string"
    || !TENANT_ACCESS_API_KEY_PATTERN.test(apiKey)
    || typeof credentialId !== "string"
    || !TENANT_ACCESS_IDENTIFIER_PATTERN.test(credentialId)
  ) return false;
  const body = document.querySelector("#one-time-credential-body");
  if (!(body instanceof HTMLElement)) return false;
  oneTimeCredentialId = credentialId;
  oneTimeCredentialReadbackVerified = readbackVerified;
  const acknowledgeAttributes = readbackVerified
    ? ""
    : ' disabled aria-disabled="true" title="签发或轮换 operation 尚未精确读回，不能确认交付"';
  body.innerHTML = `<div class="one-time-secret-warning"><span class="status-pill status-manual_review"><span class="status-icon" aria-hidden="true">!</span>仅显示一次</span><p>请立即复制并由接收方安全保管。复制后无法恢复；关闭此窗口或刷新页面后不能再次查看。</p><p>这枚凭证当前仍是“待交付”：只有确认已安全保存后才可调用 MCP。</p></div><div class="field"><label for="one-time-api-key">完整 API Key</label><input id="one-time-api-key" type="text" autocomplete="off" readonly aria-describedby="one-time-api-key-help" /></div><p id="one-time-api-key-help" class="field-help">列表只保留前缀和末四位，状态接口不会返回完整 key。</p><p id="one-time-copy-feedback" class="field-help" role="status" aria-live="polite"></p><div class="button-row one-time-secret-actions"><button class="button button-secondary" type="button" data-access-action="copy-secret">复制 API Key</button><button class="button button-primary" type="button" data-access-action="ack-secret"${acknowledgeAttributes}>确认已安全保存</button><button class="button button-secondary" type="button" data-access-action="close-secret">尚未保存，关闭并清除</button></div>`;
  const input = document.querySelector("#one-time-api-key");
  if (!(input instanceof HTMLInputElement)) {
    clearOneTimeCredentialDialog({ announce: false });
    return false;
  }
  input.value = apiKey;
  if (typeof oneTimeCredentialDialog.showModal === "function") oneTimeCredentialDialog.showModal();
  else oneTimeCredentialDialog.setAttribute("open", "");
  queueMicrotask(() => {
    input.focus();
    input.select();
  });
...
async function copyOneTimeCredential() {
  const input = document.querySelector("#one-time-api-key");
  const feedback = document.querySelector("#one-time-copy-feedback");
  if (!(input instanceof HTMLInputElement) || input.value === "") return;
  if (!navigator.clipboard || typeof navigator.clipboard.writeText !== "function") {
    if (feedback instanceof HTMLElement) feedback.textContent = "当前浏览器未提供安全复制能力，请手动复制后立即保管。";
    return;
  }
  try {
    await navigator.clipboard.writeText(input.value);
    if (feedback instanceof HTMLElement) feedback.textContent = "已复制；关闭后无法恢复，请立即安全保管。";
  } catch {
    if (feedback instanceof HTMLElement) feedback.textContent = "复制未完成，请手动复制并立即安全保管；关闭后无法恢复。";
  }
}

function accessFormText(form, name) {
  const value = new FormData(form).get(name);
  return typeof value === "string" ? value.trim() : "";
}

function accessFormTexts(form, name) {
  return new FormData(form).getAll(name).filter((value) => typeof value === "string").map((value) => value.trim()).filter(Boolean);
}

function accessInputError(detail) {
  state.accessNotice = { kind: "warning", status: "needs_input", title: "信息不完整", detail, label: "需要补充" };
  render(true);
}

async function runAccessOperation(label, operation, { showSecret = false, expectedToolNames = null } = {}) {
  if (state.accessStateLoading || !tenantAccessClient || state.mode !== "fixture") return false;
  state.accessStateLoading = true;
  state.accessStatus = "loading";
  state.accessError = null;
  state.accessNotice = { kind: "info", status: "needs_input", title: `${label}进行中`, detail: "正在等待服务端结果；没有服务端读回前不更新列表状态。", label: "等待服务端" };
  render(true);
  let oneTimeApiKey = null;
  let oneTimeResultCredentialId = null;
  let operationId = null;
  let exactReadback = false;
  try {
    const result = await operation();
    if (isRecord(result?.operation) && typeof result.operation.operation_id === "string") {
      operationId = result.operation.operation_id;
    }
    if (showSecret && isRecord(result) && typeof result.api_key === "string" && isRecord(result.credential)) {
      oneTimeApiKey = result.api_key;
      oneTimeResultCredentialId = typeof result.credential.credential_id === "string"
        ? result.credential.credential_id
        : null;
      if (expectedToolNames === null && Array.isArray(result.credential.tool_names)) {
        expectedToolNames = result.credential.tool_names.filter((toolName) => typeof toolName === "string");
      }
    }
    const readbackOk = await loadAccessState({ preserveNotice: true });
    const expectedTools = Array.isArray(expectedToolNames)
      ? [...expectedToolNames].sort()
      : null;
    const credentialReadback = expectedTools === null
      ? true
      : oneTimeResultCredentialId !== null
        && Array.isArray(state.accessState?.credentials)
        && state.accessState.credentials.some((credential) => (
          credential.credential_id === oneTimeResultCredentialId
          && JSON.stringify([...credential.tool_names].sort()) === JSON.stringify(expectedTools)
        ));
    exactReadback = readbackOk
      && operationId !== null
      && Array.isArray(state.accessState?.operations)
      && state.accessState.operations.some((item) => item.operation_id === operationId && item.status === "success")
      && credentialReadback;
    if (exactReadback) {
      state.accessStatus = "complete";
      state.accessNotice = showSecret
        ? { kind: "info", status: "needs_input", title: `${label}已写入，等待交付确认`, detail: "同一 operation ID 与精确工具清单已从服务端读回；请复制完整 Key，并确认已安全保存后再调用 MCP。", label: "待确认交付" }
        : { kind: "info", status: "success", title: `${label}已完成`, detail: "同一 operation ID 已从服务端精确读回；页面状态与服务端一致。", label: "精确读回" };
    } else {
      state.accessStatus = "manual_review";
      state.accessNotice = { kind: "warning", status: "manual_review", title: `${label}读回未完成`, detail: oneTimeApiKey ? "服务端已返回一次性凭证，请先复制；状态读回失败，当前结果需要人工确认。" : "服务端响应已收到但状态读回失败，当前结果需要人工确认。", label: "人工复核" };
    }
  } catch (error) {
    const failure = accessErrorNotice(error, label);
    state.accessStatus = failure.status;
    state.accessNotice = { kind: "warning", status: failure.status, title: `${label}未完成`, detail: failure.detail, label: failure.status === "blocked" ? "已阻断" : failure.status === "manual_review" ? "人工复核" : "未完成" };
    if (failure.status === "unavailable" || failure.status === "needs_input") state.accessError = failure.detail;
  } finally {
    state.accessStateLoading = false;
    render(true);
    if (oneTimeApiKey !== null) {
      if (!openOneTimeCredentialDialog(oneTimeApiKey, oneTimeResultCredentialId, { readbackVerified: exactReadback })) {
        state.accessNotice = { kind: "warning", status: "manual_review", title: `${label}的凭证未显示`, detail: "服务端一次性凭证未通过格式校验，页面未显示任何未经核验的 key。", label: "人工复核" };
        render(true);
      }
    }
  }
...
async function handleAccessFormSubmit(form) {
  if (state.mode !== "fixture") {
    accessInputError("正式路径写操作固定关闭；当前页面不提交写请求。");
    return;
  }
  if (!tenantAccessClient || !state.accessActor || !state.accessState) return;
  if (form.id === "access-create-tenant-form") {
    const tenantId = accessFormText(form, "tenant_id");
    const displayName = accessFormText(form, "display_name");
    if (tenantId === "" || displayName === "") {
      accessInputError("请补充租户标识和租户名称。");
      return;
    }
    form.reset();
    await runAccessOperation("创建租户", () => tenantAccessClient.createTenant({
      schema_version: TENANT_ACCESS_SCHEMA_VERSION,
      tenant_id: tenantId,
  ...
  if (form.id !== "access-issue-form") return;
  const issueTenantId = accessFormText(form, "tenant_id");
  const clientId = accessFormText(form, "client_id");
  const label = accessFormText(form, "label");
  const toolNames = accessFormTexts(form, "tool_names");
  const expiresInSeconds = Number(accessFormText(form, "expires_in_seconds"));
  if (issueTenantId === "" || clientId === "" || label === "" || toolNames.length === 0 || !Number.isSafeInteger(expiresInSeconds)) {
    accessInputError("请补充客户端信息并至少选择一项插件功能。");
    return;
  }
  form.reset();
  await runAccessOperation("签发凭证", () => tenantAccessClient.issueCredential({
    schema_version: TENANT_ACCESS_SCHEMA_VERSION,
    tenant_id: issueTenantId,
    client_id: clientId,
    label,
    tool_names: toolNames,
    expires_in_seconds: expiresInSeconds,
  }, undefined), { showSecret: true });
}

async function handleTenantAccessAction(target) {
  ...
  if (action === "open-identity") {
    openAccessIdentityDialog();
    return;
  }
  if (action === "close-tool-editor") {
    closeCredentialToolDialog();
    return;
  }
  if (action === "fixture-identity") {
    const identity = FIXTURE_IDENTITIES.find((item) => item.actor === target.dataset.identity);
    if (state.mode === "fixture" && isFixtureIdentityVisible(window.location.search)) bindAccessIdentity(identity);
    return;
  }
  if (action === "copy-secret") {
    await copyOneTimeCredential();
    return;
  }
  if (action === "close-secret") {
    clearOneTimeCredentialDialog();
    return;
  }
  if (action === "refresh") {
    await loadAccessState({ announce: true });
    return;
  }
  if (action === "create-tenant" || action === "issue-credential") return;
  if (!state.accessActor || !state.accessState || state.mode !== "fixture" || !tenantAccessClient) return;
  if (action === "ack-secret") {
    const credentialId = oneTimeCredentialId;
    if (credentialId === null || !oneTimeCredentialReadbackVerified) {
      state.accessNotice = { kind: "warning", status: "manual_review", title: "暂不能确认交付", detail: "签发或轮换 operation 尚未精确读回；请保留人工复核状态。", label: "人工复核" };
      render(true);
      return;
    }
    const acknowledged = await runAccessOperation("确认安全交付", () => tenantAccessClient.acknowledgeCredentialDelivery(credentialId, {
      schema_version: TENANT_ACCESS_SCHEMA_VERSION,
      reason_code: "operator_confirmed_secure_storage",
    }));
    if (acknowledged) clearOneTimeCredentialDialog({ acknowledged: true, announce: false });
    return;
  }
  if (action === "set-tenant-status") {
    const tenantId = target.dataset.tenantId;
    const status = target.dataset.tenantStatus;
    if (!tenantId || (status !== "active" && status !== "suspended")) return;
    await runAccessOperation(status === "active" ? "恢复租户" : "暂停租户", () => tenantAccessClient.setTenantStatus(tenantId, {
      schema_version: TENANT_ACCESS_SCHEMA_VERSION,
      status,
  ...
  if (action === "rotate-credential") {
    openCredentialToolDialog(credentialId);
    return;
  }
  if (action === "revoke-credential") {
    await runAccessOperation("吊销凭证", () => tenantAccessClient.revokeCredential(credentialId, {
      schema_version: TENANT_ACCESS_SCHEMA_VERSION,
...
document.addEventListener("click", (event) => {
  ...
  if (!target) return;
  if (target.dataset.accessAction) {
    void handleTenantAccessAction(target);
    return;
  }
  ...
  if (view) {
    ...
    render(true);
    focusMain();
    if (view === "tenant-access" && state.accessActor && state.accessState === null) void loadAccessState({ announce: true });
    return;
  }
...
document.addEventListener("change", (event) => {
  ...
  if (
    ...
    && target.form?.id === "credential-tool-form"
    && target.name === "tool_names"
  ) {
    const error = document.querySelector("#credential-tool-form-error");
    if (error instanceof HTMLElement && accessFormTexts(target.form, "tool_names").length > 0) {
      error.hidden = true;
    }
    return;
  }
...
document.addEventListener("submit", (event) => {
  ...
  if (form.id === "credential-tool-form") {
    event.preventDefault();
    void submitCredentialToolForm(form);
    return;
  }
  if (!["access-create-tenant-form", "access-issue-form"].includes(form.id)) return;
  event.preventDefault();
  void handleAccessFormSubmit(form);
});

document.querySelector("#access-identity-form")?.addEventListener("submit", (event) => {
  event.preventDefault();
  submitAccessIdentity();
});
...
accessIdentityDialog?.addEventListener("cancel", (event) => {
  event.preventDefault();
  closeAccessIdentityDialog();
});

credentialToolDialog?.addEventListener("cancel", (event) => {
  event.preventDefault();
  closeCredentialToolDialog();
});

oneTimeCredentialDialog?.addEventListener("cancel", (event) => {
  event.preventDefault();
  clearOneTimeCredentialDialog();
});
