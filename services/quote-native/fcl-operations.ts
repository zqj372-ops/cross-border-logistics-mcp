...
/** Only the separate delivery tariff retains a service window; maintenance dates do not. */
export function fclDeliveryValidity(calculation:Pick<FclEstimateCalculation,'lines'>){
  const delivery=calculation.lines.filter(line=>line.id.startsWith('delivery:'));
  return {
    valid_from:delivery.map(line=>line.valid_from).filter((value):value is string=>value!==null).sort().at(-1)??null,
    valid_until:delivery.map(line=>line.valid_until).filter((value):value is string=>value!==null).sort()[0]??null,
  };
}
...
export function calculateFclEstimate(dataset:FclRateDataset,request:FclEstimateRequest,rateId:string,templateId:string):FclEstimateCalculation {
  ...
  const templates=ops.templates.filter(t=>t.id===templateId&&t.enabled);
  ...
  if(templates.length!==1)blockers.push(templates.length?'template_ambiguous':'template_unavailable');
  ...
  const addCharge=(chargeId:string)=>{
    ...
    const choices=ops.charges.filter(v=>v.id===chargeId);
    if(choices.length!==1){blockers.push(`charge_${choices.length?'ambiguous':'unavailable'}:${chargeId}`);return;}
    const c=choices[0]!;
    if(isBaseOceanFreight(c)){
      if(c.ocean_freight_resolution?.action!=='exclude')blockers.push(`legacy_ocean_fee_review:${c.id}`);
      return;
    }
    ...
    for(const box of boxes)add({id:`charge:${c.id}:${box.type??'shipment'}`,code:c.code,name_zh:c.name_zh,name_en:c.name_en,category:c.category,source_ref:c.source_ref,source_version:c.source_version,valid_from:null,valid_until:null,unit:c.unit,container_type:box.type,quantity:String(box.quantity),cost_price:c.amount,sell_price:c.sell_amount??selling(c.amount,template.margin_rule),currency:c.currency,editable:c.editable});
  };
  ...
  rate.additional_fees.forEach((fee,index)=>{
    if(isBaseOceanFreight(fee)){if(fee.ocean_freight_resolution?.action!=='exclude')blockers.push(`legacy_rate_ocean_fee_review:${index}`);return;}
    if(template.charge_ids.some(id=>ops.charges.some(c=>c.id===id&&(c.name_zh===fee.name||c.name_en===fee.name)))){blockers.push(`legacy_rate_fee_collision:${index}`);return;}
  ...
  const {valid_from:validFrom,valid_until:validUntil}=fclDeliveryValidity({lines});
...
function pickSource(value:Pick<FclCharge,'source_ref'|'source_version'|'valid_from'|'valid_until'>){return {source_ref:value.source_ref,source_version:value.source_version,valid_from:value.valid_from??null,valid_until:value.valid_until??null};}
