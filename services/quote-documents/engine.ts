...
export function renderHtml(d:QuoteDocument,t:QuoteTemplate,approved:boolean,fclMetadata?:FclRenderMetadata){
 ...
 const serviceLabels:Record<string,string>={pickup:'提货',export_customs:'出口报关',ocean_freight:'海运费',canada_customs:'加拿大清关',devanning_storage:'拆柜/仓储',delivery:'派送'};
 ...
 const fclRows=visible.map(f=>`<tr><td>${e(f.group)} · ${e(f.name)}</td><td class="num">${e(f.quantity)}</td><td>${e(f.unit==='CNTR'?'柜':f.unit==='SHIPMENT'?'票':f.unit)}</td><td class="num">${e(new D(f.unit_price).toDecimalPlaces(2,D.ROUND_HALF_UP).toFixed())}</td><td>${f.currency}</td><td class="num strong">${f.amount}</td><td>${e(f.note)}</td></tr>`).join('')+[...merged.values()].map(f=>`<tr><td colspan="5">${e(f.name)}</td><td>${f.currency}</td><td class="num strong">${f.amount.toFixed(2)}</td></tr>`).join('');
 ...
 <h2>费用明细</h2><table><thead>${fclMetadata?'<tr><th style="width:20%">费用项目</th><th style="width:6%">数量</th><th style="width:10%">单位</th><th style="width:12%">单价</th><th style="width:6%">币种</th><th style="width:13%">小计</th><th style="width:33%">说明</th></tr>':'<tr><th style="width:17%">费用项目</th><th style="width:16%">说明</th><th>数量</th><th>单位</th><th style="width:12%">单价</th><th>币种</th><th style="width:13%">小计</th><th>备注</th></tr>'}</thead><tbody>${rows}</tbody></table>
 <h2>费用合计 / Summary</h2><div class="totals"><div class="total-card"><h3>原币种合计</h3>${currencies.map(c=>`<div class="total-line"><span>${c}</span><strong>${result.by_currency[c]}</strong></div>`).join('')}</div>${fclMetadata?fclConversionCards:`<div class="total-card"><h3>折算人民币</h3><strong>${result.total_cny===null?'未填写完整汇率':result.total_cny+' CNY'}</strong></div><div class="total-card"><h3>折算美元</h3><strong>${result.total_usd===null?'未填写完整汇率':result.total_usd+' USD'}</strong></div>`}</div>
 ${fclMetadata?fclRateParts.length>0?`<p>换算率：${fclRateParts.map(e).join('；')}</p>`:'':`<p>填写汇率：1 USD = ${e(d.exchange_rates.USD??'未填写')} CNY；1 CAD = ${e(d.exchange_rates.CAD??'未填写')} CNY</p>`}
