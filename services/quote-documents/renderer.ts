...
export async function renderPdf(html:string):Promise<Buffer>{
 ...
 await exec(binary,['--headless','--disable-gpu','--disable-background-networking','--no-first-run','--no-pdf-header-footer',`--user-data-dir=${join(dir,'profile')}`,`--print-to-pdf=${output}`,pathToFileURL(input).href],{env:{...process.env,XDG_CONFIG_HOME:join(dir,'config'),XDG_CACHE_HOME:join(dir,'cache')},timeout:10000,killSignal:'SIGKILL',maxBuffer:65536});
