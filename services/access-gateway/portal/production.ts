...
let nativeStore:NativeAdminStore|undefined,nativeAdminService:NativeAdminService|undefined,nativeFreightcom:NativeFreightcomService|undefined,scheduleLive:ReturnType<typeof createProductionScheduleLiveService>|undefined,fclDependencies:FclHttpDependencies|undefined;
 ...
 if(casesEnabled||fclEnabled){if(shared)throw new Error("cases_shared_store_unavailable");const path=resolve(root,"business-cases.sqlite"),version=readPortalProductionDatabaseVersion(path,"freightclaw-business-cases");if(fclEnabled&&version!==2)throw new Error("fcl_schema_not_upgraded");caseStore=new CaseStore(path,version===2?{fcl:{mode:"reopen" as const}}:undefined);if(!fclEnabled&&casesEnabled)caseService=new CaseService(caseStore,portal);}
 ...
 if(scheduleLiveTenants.length>0){const evidenceRoot=environment.PORTAL_SCHEDULE_LIVE_EVIDENCE_ROOT?.trim()||resolve(root,"schedule-collector","evidence"),auditPath=environment.PORTAL_SCHEDULE_LIVE_AUDIT_PATH?.trim()||resolve(root,"schedule-collector","audit.jsonl");if(!isAbsolute(evidenceRoot)||!isAbsolute(auditPath))throw new Error("schedule_live_path_must_be_absolute");scheduleLive=createProductionScheduleLiveService({portal,audit:new JsonlScheduleLiveAuditSink(auditPath),evidenceRoot,tenantAllowlist:scheduleLiveTenants,carrierAllowlist:parseScheduleLiveCarrierAllowlist(environment.PORTAL_SCHEDULE_LIVE_CARRIER_ALLOWLIST)});}
 ...
 if(fclEnabled){
  ...
  const composed=await composeProductionFcl({portal,caseStore,nativeStore,documentStore,documentWorkflowStore,receiverSub:fclReceiverSub!,authority:fclAuthority,mailTransport:fclMailTransport,caseCredentialSecret:fclCaseCredentialSecret!,publicSessionSecret:fclPublicSessionSecret!,...(nativeEnabled?{nativeBusiness:{business,current:org=>nativeStore!.current(org,'residential')}}:{})});
  caseService=composed.caseService;nativeAdminService=composed.nativeAdmin;documentService=composed.documentService;documentWorkflowService=composed.documentWorkflow;fclDependencies=composed.fcl;
 }
