...
// New contracts have separate versions and never widen legacy T0 schemas.
const { applicationMcpExchangeSchema } = await import("../../services/access-gateway/portal/business-access/mcp");
const { APPLICATION_MCP_TOOLS } = await import("../../src/logistics_mcp/platform/application-tools");
const { callEventSchema, callQuerySchema } = await import("../../services/access-gateway/portal/call-log");
const { customsHistoryListInput, customsHistoryGetInput, customsHistoryListResponse, customsHistoryGetResponse } = await import("../../services/access-gateway/portal/business/customs-history-client");
const { signedProviderReleaseSchema } = await import("../../src/logistics_mcp/module-runtime/managed-provider");
const emit=(name:string,schema:z.ZodType)=>output(`${name}.schema.json`,{$schema:"https://json-schema.org/draft/2020-12/schema",$id:`https://freightclaw.local/schemas/${name}.schema.json`,...jsonSchema(schema)});
const newExchange=jsonSchema(applicationMcpExchangeSchema);(newExchange.properties as JsonObject).requested_tool_names={type:"array",minItems:1,maxItems:8,uniqueItems:true,items:{enum:APPLICATION_MCP_TOOLS}};
output("application-mcp-exchange-request.schema.json",{$schema:"https://json-schema.org/draft/2020-12/schema",$id:"https://freightclaw.local/schemas/application-mcp-exchange-request.schema.json",...newExchange});
const newSuccess=z.object({schema_version:z.literal("application-mcp-access@2026-09-06.v1"),status:z.literal("success"),data:z.object({access_token:z.string().min(1),token_type:z.literal("Bearer"),expires_in:z.literal(300),mcp_profile:z.literal("business-v1"),tool_names:z.array(z.enum(APPLICATION_MCP_TOOLS)).min(1).max(8)}).strict(),reason_codes:z.array(z.string()).max(0)}).strict();
const newError=z.object({schema_version:z.literal("application-mcp-access@2026-09-06.v1"),status:z.enum(["blocked","unavailable"]),data:z.null(),reason_codes:z.array(z.string().min(1)).min(1)}).strict();
emit("application-mcp-exchange-response",z.union([newSuccess,newError]));
emit("portal-call-query",callQuerySchema);emit("portal-call-event",callEventSchema);
const states=["success","needs_input","manual_review","blocked","unavailable"] as const;
emit("portal-call-page",z.object({schema_version:z.literal("portal-call-log@2026-09-06.v1"),status:z.literal("success"),data:z.object({events:z.array(callEventSchema).max(100),next_cursor:z.string().nullable(),summary:z.object({total:z.number().int().nonnegative(),status_counts:z.object(Object.fromEntries(states.map(s=>[s,z.number().int().nonnegative()]))).strict(),average_duration_ms:z.number().int().nonnegative()}).strict(),window:z.object({from:z.string().datetime(),to:z.string().datetime(),retention_days:z.literal(30),maximum_events_per_tenant:z.number().int().positive().max(100000)}).strict()}).strict(),reason_codes:z.array(z.string()).max(0)}).strict());
emit("customs-history-list-request",customsHistoryListInput);emit("customs-history-get-request",customsHistoryGetInput);emit("customs-history-list-response",customsHistoryListResponse);emit("customs-history-get-response",customsHistoryGetResponse);emit("provider-release",signedProviderReleaseSchema);
