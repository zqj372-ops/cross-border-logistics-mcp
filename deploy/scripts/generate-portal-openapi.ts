...
export function generatePortalOpenApi(): ObjectValue {
  ...
  for (const directory of ["docs/contracts/schemas", "schemas/access-gateway"]){
    for (const file of readdirSync(directory).filter(name=>name.endsWith(".schema.json"))){
      if(directory.endsWith("access-gateway")&&!/^(?:application-(?:mcp-)?exchange-(?:request|response)|portal-call-(?:query|page|event)|portal-cases-response(?:-v2)?|customs-history-(?:list|get)-(?:request|response)|business-(?:call|exchange)-(?:request|response)|exchange-(?:request|response)|error-envelope|jwks-response)\./u.test(file))continue;
  ...
  const linkedSchemaNames:Record<string,string>={};
  for(const file of readdirSync("schemas/admin-control/quote-documents").filter(name=>/-v2(?:-response)?\.schema\.json$/u.test(name))){
    const base=file.replace(/\.schema\.json$/u,""),camel=base.split("-").map(part=>part[0]!.toUpperCase()+part.slice(1)).join(""),name=base.startsWith("linked-error")?"LinkedErrorV2Response":"Linked"+camel;
