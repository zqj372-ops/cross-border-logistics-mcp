...
const INITIALIZER_EXPORT = "initializeSqliteTenantAccessState";
...
export function assertNoArguments(args) {
  if (!Array.isArray(args) || args.length !== 0) {
    throw new Error("init-tenant-access-fixture does not accept command-line arguments");
  }
}
...
async function loadOfficialInitializer(root) {
  ...
  try {
    ...
    throw new Error(
      "The built start entry is unavailable; run npm run build before initializing Tenant Access.",
    );
  }
  const initializer = builtEntry[INITIALIZER_EXPORT];
  if (typeof initializer !== "function") {
    throw new Error("The built start entry must export initializeSqliteTenantAccessState.");
  }
  return initializer;
}

export async function initializeFixtureTenantAccessState() {
  const root = applicationRoot();
  const initializeSqliteTenantAccessState = await loadOfficialInitializer(root);
...
if (isMainModule()) {
  try {
    ...
    await initializeFixtureTenantAccessState();
  } catch (error) {
    console.error(error instanceof Error ? error.message : "Tenant Access initialization failed.");
    process.exitCode = 1;
  }
}
