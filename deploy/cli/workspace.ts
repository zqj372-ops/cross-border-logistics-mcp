...
export async function runWorkspace(args:string[],io:CliIO,helpers:Helpers):Promise<number>{
 ...
 try{
  ...
  let input:unknown={};if(values.input){input=helpers.parseJson(values.input==='-'?await helpers.readStdin((io.stdin??process.stdin) as NodeJS.ReadStream,name.startsWith("customs-data ")||name.startsWith("residential-rates ")?16*1024*1024:name==='quote extract'?131072:15000):await helpers.readFileBounded(values.input,name.startsWith("customs-data ")||name.startsWith("residential-rates ")?16*1024*1024:name==='quote extract'?131072:32768,name==="freightcom save"));}if(!input||typeof input!=='object'||Array.isArray(input))throw new Failure('input_invalid');
