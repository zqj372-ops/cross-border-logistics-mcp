...
for(const action of ['config-save','preview','save','list','get','approve','export']) schemas['documents '+action]=quoteDocumentSchemas[(action==='export'?'get':action) as keyof typeof quoteDocumentSchemas];
...
export async function runWorkspace(args:string[],io:CliIO,helpers:Helpers):Promise<number>{
 ...
 try{
  ...
  let input:unknown={};if(values.input){input=helpers.parseJson(values.input==='-'?await helpers.readStdin((io.stdin??process.stdin) as NodeJS.ReadStream,name.startsWith("customs-data ")||name.startsWith("residential-rates ")?16*1024*1024:name==='quote extract'||name.startsWith('documents ')?131072:15000):await helpers.readFileBounded(values.input,name.startsWith("customs-data ")||name.startsWith("residential-rates ")?16*1024*1024:name==='quote extract'||name.startsWith('documents ')?131072:32768,name==="freightcom save"));}if(!input||typeof input!=='object'||Array.isArray(input))throw new Failure('input_invalid');
  if(!validCaseInput(name,input)||schemas[name]&&!schemas[name].safeParse(input).success)throw new Failure('input_schema_invalid');
  if(command[1]==='POST'&&!path.startsWith('/business/')&&!['documents list','documents get','documents preview','documents export'].includes(name)&&(!values['idempotency-key']||!/^[A-Za-z0-9._:-]{16,128}$/u.test(values['idempotency-key'])))throw new Failure('idempotency_key_required');
